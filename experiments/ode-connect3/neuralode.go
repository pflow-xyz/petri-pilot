// A REAL Neural ODE, as distinct from everything else in this experiment.
// Every other evaluator here is classical system identification: a fixed
// mass-action RHS dictated by the declared Petri net's stoichiometry, with a
// handful of physically-meaningful scalar rates fit to data. This file
// throws that structure away. dx/dt = MLP(x; theta) — an unconstrained
// function approximator with no wiring saying which cells threaten which
// win-line, no catalytic-copy structure, nothing hand-derived. The entire
// "policy-in-structure" idea from ode-minimax/ode-connect3's other files is
// exactly what this evaluator has to learn from scratch, if it learns it at
// all.
//
// Scope, stated honestly: the state actually integrated is just the 4
// quantities every evaluator in this package reads out of a final state —
// win_x, win_o, x_turn, o_turn — conditioned on the 48 raw board features
// (gravity tokens + x/o marks) held FIXED across the horizon. This isn't the
// full 52-place system evolving under the MLP; it's the observable
// sub-state evolving under an MLP that sees the whole board. That keeps the
// parameter count (and training cost) sane while still being a genuine test
// of the representational question: can an unconstrained function of the
// full board learn the future-support predicate finding 6 named, in a way
// fixed-form catalytic rates could not (finding 7)?
//
// Training is discretize-then-optimize: forward-integrate with a fixed-step
// Euler scheme, then backprop exactly through the unrolled Euler steps
// (store each step's 4-dim state, reverse-mode through the two dense
// layers). This is NOT go-pflow's continuous adjoint (SolveAdjoint) — that
// machinery is built for the RateFunc/mass-action framework and doesn't
// apply to an arbitrary vector-valued RHS. Backprop-through-the-unrolled-
// solver is a standard, simpler way to train small Neural ODEs and is exact
// for the discretization actually used at inference time (no approximation
// gap between what's optimized and what's evaluated). checkNeuralGrad
...
// boardDim (48, the raw cell/gravity features) + ndStateDim (the current
// dynamic state fed back in, mass-action's "state-dependent rate" analog).
type mlp struct {
	boardDim, h int
...
// boardFeatures extracts the 48 raw board features (p<c>, x<c>, o<c> per
...
// ndHorizonSteps/ndDt mirror odeHorizon (0.5) at a fixed Euler step count.
// Cheap enough (two small dense layers, no adaptive solver) that a
// generous step count costs nothing.
const ndSteps = 40

func ndDt() float64 { return odeHorizon / ndSteps }

// ndScore integrates from x0 (built from the marking: win_x=win_o=0,
// x_turn/o_turn read off mk) and returns the same score convention every
// other evaluator here uses, plus the raw trajectory (for a training call's
// backward pass) or nil (for play/referee, where no gradient is needed).
...
func ndScoreGrad(maximizes bool, lam float64) (dLdxFinal []float64, dsdlam float64) {
	...
	return dLdxFinal, 0 // dsdlam filled by caller from f[ndWinO]
}

// neuralODEPlayer: for each candidate, integrate the learned dynamics from
// the post-move marking and score exactly as odePlayer would from an ODE
// final read — the leaf logic is unchanged, only the dynamics engine is.
...
// rankLossNeural / hingeSubgradFlat mirror fitgrad.go's pattern (evaluate
// every candidate, find the best-preferred/best-non-preferred pair per
// decision, accumulate their score-gradient difference for any decision the
// hinge margin doesn't already satisfy) but generalized to an arbitrary flat
// parameter vector rather than a handful of tied scalars.
func evalNeuralDecisions(m *model, n *mlp, positions []trainPos, lam float64) (decisions []learn.RankedDecision, grads [][][]float64, dlams [][]float64) {
	for _, p := range positions {
		...
		for i, mv := range p.moves {
			...
			dLdxFinal, _ := ndScoreGrad(p.maximizes, lam)
			grad, _ := n.backward(board, traj, dLdxFinal)
			g[i] = grad
			if !p.maximizes {
				dl[i] = xf[ndWinO]
			}
...
func fitNeuralODE(m *model, positions []trainPos, h, iters int, l2 float64, verbose bool) (n *mlp, lam float64) {
	...
	fg := func(u []float64) (float64, []float64) {
		...
		// which lives in its own log-space coordinate and already has a
		// natural scale from the exp chain rule below).
