...
function renderPokerTable() {
  ...
  gameState.players.forEach((player, i) => {
    ...
    seatEl.classList.remove('heatmap-strong', 'heatmap-medium', 'heatmap-weak')
    let strengthEl = seatEl.querySelector('.heatmap-strength')

    if (heatmapActive && player.cards && player.cards.length >= 2 && !player.folded) {
      // Calculate hand strength for this player
      const strength = evaluateHandStrength(player.cards, gameState.communityCards)

      // Apply color class based on strength
      if (strength >= 0.6) {
        seatEl.classList.add('heatmap-strong')
      } else if (strength >= 0.35) {
        seatEl.classList.add('heatmap-medium')
      } else {
        seatEl.classList.add('heatmap-weak')
      }

      // Add/update strength indicator
      if (!strengthEl) {
        strengthEl = document.createElement('div')
        strengthEl.className = 'heatmap-strength'
        seatEl.appendChild(strengthEl)
      }
      strengthEl.textContent = `${(strength * 100).toFixed(0)}%`
...
function renderEventHistory() {
  ...
  listEl.innerHTML = recentEvents.reverse().map((event, reverseIndex) => {
    // Calculate sequence number (most recent = total, oldest shown = total - 9)
    const seqNum = totalEvents - reverseIndex
    ...
    return `
      <div class="event-item">
        <span style="opacity: 0.5; font-size: 0.8rem;">#${seqNum}</span>
...
/**
 * Render strategic analysis - shows hand strength, draws, and pot odds
 */
function renderODEAnalysis() {
  const analysisEl = document.getElementById('ode-analysis')

  // Check if we have cards to analyze
  const playerCards = gameState.players[0].cards
  const hasCards = playerCards && playerCards.length >= 2
  ...
  if (!hasCards) {
    if (gameState.currentRound === 'waiting') {
      ...
      analysisEl.innerHTML = '<p style="color: rgba(255,255,255,0.6); font-style: italic;">Waiting for cards...</p>'
    }
    return
  }

  // Get hand evaluation
  const evaluation = evaluatePokerHand(playerCards, gameState.communityCards)
  const draws = calculateDraws(playerCards, gameState.communityCards)

  // Calculate pot odds
  const callAmount = Math.max(0, gameState.currentBet - gameState.players[0].bet)
  const potOdds = callAmount > 0 ? (callAmount / (gameState.pot + callAmount) * 100).toFixed(0) : 0

  // Determine hand strength color
  const strengthColor = evaluation.strength >= 0.6 ? '#10b981' :
                        evaluation.strength >= 0.35 ? '#f59e0b' : '#ef4444'

  // Build analysis HTML
  let html = `
    <div style="margin-bottom: 0.75rem;">
      <div style="font-size: 1.1rem; font-weight: bold; color: ${strengthColor};">
  ...
  // Show draws if we're not at the river
  const communityCount = (gameState.communityCards.flop?.length || 0) +
                         (gameState.communityCards.turn?.length || 0) +
                         (gameState.communityCards.river?.length || 0)

  if (communityCount > 0 && communityCount < 5 && draws.outs > 0) {
    html += `<div style="margin-bottom: 0.75rem; padding: 0.5rem; background: rgba(255,255,255,0.05); border-radius: 6px;">`
    html += `<div style="font-weight: bold; margin-bottom: 0.25rem;">Draws:</div>`

    if (draws.flushDraw > 0) {
      html += `<div style="font-size: 0.85rem;">♠ Flush draw (${draws.flushDraw} outs)</div>`
    }
    if (draws.straightDraw > 0) {
      const drawType = draws.straightDraw === 8 ? 'Open-ended' : 'Gutshot'
      html += `<div style="font-size: 0.85rem;">↔ ${drawType} straight (${draws.straightDraw} outs)</div>`
    }

    html += `<div style="margin-top: 0.5rem; font-size: 0.85rem; color: #f59e0b;">
      Total: ${draws.outs} outs → ~${(draws.equity * 100).toFixed(0)}% to improve
    </div>`
    html += `</div>`
  }

  // Show pot odds if there's a bet to call
  if (callAmount > 0) {
    const shouldCall = draws.equity * 100 > potOdds || evaluation.strength > 0.5
    const oddsColor = shouldCall ? '#10b981' : '#ef4444'

    html += `<div style="padding: 0.5rem; background: rgba(255,255,255,0.05); border-radius: 6px;">`
    html += `<div style="font-weight: bold; margin-bottom: 0.25rem;">Pot Odds:</div>`
    html += `<div style="font-size: 0.85rem;">Call $${callAmount} into $${gameState.pot} pot</div>`
    html += `<div style="font-size: 0.85rem;">Need ${potOdds}% equity to call</div>`
    html += `<div style="margin-top: 0.25rem; color: ${oddsColor}; font-weight: bold;">
      ${shouldCall ? '✓ Odds favor calling' : '✗ Fold unless bluffing'}
    </div>`
    html += `</div>`
  }
...
/**
 * Toggle heatmap coloring on player seats
 */
function toggleHeatmap() {
  ...
  btn.textContent = heatmapActive ? 'Hide Heatmap' : 'Toggle Heatmap'
  btn.classList.toggle('active', heatmapActive)

  // Re-render the table to apply/remove heatmap coloring
  renderPokerTable()

  if (heatmapActive && gameState.currentRound !== 'waiting') {
    showStatus('Heatmap: Green=Strong, Yellow=Medium, Red=Weak', 'info')
  }
}
