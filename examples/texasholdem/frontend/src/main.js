...
function hasCustomDashboard() {
  return customElements.get('texasholdem-dashboard') !== undefined
}
...
const TRANSITION_DEFS = [
  ...
  },
  {
    id: 'deal_flop',
    name: 'Deal Flop',
    description: "Deal the flop (3 community cards)",
    fields: [
    ]
  },
  {
    id: 'deal_turn',
    name: 'Deal Turn',
    description: "Deal the turn (1 community card)",
    fields: [
    ]
  },
  {
    id: 'deal_river',
    name: 'Deal River',
    description: "Deal the river (1 community card)",
    fields: [
    ]
  },
  {
    id: 'go_showdown',
    name: 'Go Showdown',
    description: "Proceed to showdown",
    fields: [
    ]
  },
  {
  ...
  },
  {
    id: 'advance_to_p1',
    name: 'Advance To P1',
    description: "Pass turn to player 1",
    fields: [
    ]
  },
  {
    id: 'advance_to_p2',
    name: 'Advance To P2',
    description: "Pass turn to player 2",
    fields: [
    ]
  },
  {
    id: 'advance_to_p3',
    name: 'Advance To P3',
    description: "Pass turn to player 3",
    fields: [
    ]
  },
  {
    id: 'advance_to_p4',
    name: 'Advance To P4',
    description: "Pass turn to player 4",
    fields: [
    ]
  },
  {
    id: 'advance_to_p0',
    name: 'Advance To P0',
    description: "Pass turn to player 0",
    fields: [
    ]
  },
  {
    id: 'end_betting_round',
    name: 'End Betting Round',
    description: "End the betting round",
    fields: [
    ]
  },
  {
...
const api = {
  ...
  async getInstance(id) {
    const response = await fetch(`${API_BASE}/api/texasholdem/${id}`, { headers: getHeaders() })
    return handleResponse(response)
  },

  async createInstance(data = {}) {
    const response = await fetch(`${API_BASE}/api/texasholdem`, {
      method: 'POST',
      headers: getHeaders(),
...
async function renderListPage() {
  ...
  if (hasCustomDashboard()) {
    app.innerHTML = `<texasholdem-dashboard></texasholdem-dashboard>`
    return
  }
...
function renderInstancesList() {
  ...
  container.innerHTML = instances.map(inst => {
    ...
    return `
      <div class="entity-card" onclick="navigate('/texas-holdem/${inst.id}')">
        ...
        <div class="entity-actions">
          <button class="btn btn-sm btn-secondary" onclick="event.stopPropagation(); navigate('/texas-holdem/${inst.id}')">
            View
          </button>
        </div>
      </div>
...
async function loadPetriView() {
  ...
  if (!document.querySelector('link[href*="petri-view.css"]')) {
    ...
    css.rel = 'stylesheet'
    css.href = 'https://cdn.jsdelivr.net/gh/pflow-xyz/pflow-xyz@latest/public/petri-view.css'
  ...
  if (!customElements.get('petri-view')) {
    await new Promise((resolve, reject) => {
      ...
      script.type = 'module'
      script.src = 'https://cdn.jsdelivr.net/gh/pflow-xyz/pflow-xyz@latest/public/petri-view.js'
              ...
              ${instances.slice(0, 20).map(inst => {
                ...
                return `
                  <tr>
                    ...
                    <td><button class="btn btn-sm btn-link" onclick="navigate('/texas-holdem/${inst.id}')">View</button></td>
                  </tr>
                `
              }).join('')}
            </tbody>
...
window.handleSubmitCreate = async function(event) {
  ...
  try {
    ...
    navigate(`/texas-holdem/${result.aggregate_id || result.id}`)
  } catch (err) {
...
function handleRouteChange(event) {
  ...
  if (path === '/texas-holdem' || path === '/') {
    renderListPage()
  } else if (path === '/texas-holdem/new') {
    renderFormPage()
  } else if (path === '/texas-holdem/:id') {
    renderDetailPage()
  } else if (path === '/schema') {
    renderSchemaPage()
  } else {
