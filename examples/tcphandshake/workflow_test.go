...
func TestWorkflowConstants(t *testing.T) {
	...
	places := AllPlaces()
	if len(places) != 8 {
		t.Errorf("expected 8 places, got %d", len(places))
	}

	// Verify transition constants
	transitions := AllTransitions()
	if len(transitions) != 6 {
		t.Errorf("expected 6 transitions, got %d", len(transitions))
	}
}
...
func TestInitialPlaces(t *testing.T) {
	...
	if initial[PlaceClientClosed] != 1 {
		t.Errorf("expected PlaceClientClosed initial = 1, got %d", initial[PlaceClientClosed])
	}
	if initial[PlaceServerListen] != 1 {
		t.Errorf("expected PlaceServerListen initial = 1, got %d", initial[PlaceServerListen])
	}
}
...
func TestInitialState(t *testing.T) {
	...
	if places[PlaceClientClosed] != 1 {
		t.Errorf("expected PlaceClientClosed = 1, got %d", places[PlaceClientClosed])
	}
	if places[PlaceServerListen] != 1 {
		t.Errorf("expected PlaceServerListen = 1, got %d", places[PlaceServerListen])
	}
}
