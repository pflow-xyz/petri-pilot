...
func NewAggregate(id string) *Aggregate {
	...
	sm.AddTransition(eventsource.Transition{
		ID:        TransitionCatalyze,
		EventType: EventTypeCatalyze,
		Inputs: map[string]int{
			PlaceComplex: 1,
		},
		Outputs: map[string]int{
	...
	sm.RegisterHandler(EventTypeBind, func(state *State, event *eventsource.Event) error {
		return applyBind(state, event)
	})
	sm.RegisterHandler(EventTypeUnbind, func(state *State, event *eventsource.Event) error {
		return applyUnbind(state, event)
	})
	sm.RegisterHandler(EventTypeCatalyze, func(state *State, event *eventsource.Event) error {
		return applyCatalyze(state, event)
	})
...
func applyBind(state *State, event *eventsource.Event) error {
...
func applyUnbind(state *State, event *eventsource.Event) error {
	// No data transformations for this transition
	_ = state
	_ = event
	return nil
...
func applyCatalyze(state *State, event *eventsource.Event) error {
	// No data transformations for this transition
	_ = state
	_ = event
	return nil
