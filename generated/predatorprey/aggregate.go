...
func NewAggregate(id string) *Aggregate {
	...
	sm.AddTransition(eventsource.Transition{
		ID:        TransitionPreyReproduce,
		EventType: EventTypePreyReproduce,
	...
	sm.AddTransition(eventsource.Transition{
		ID:        TransitionPredatorDeath,
		EventType: EventTypePredatorDeath,
		Inputs: map[string]int{
			PlacePredator: 1,
		},
		Outputs: map[string]int{
	...
	sm.RegisterHandler(EventTypePreyReproduce, func(state *State, event *eventsource.Event) error {
		return applyPreyReproduce(state, event)
	})
	sm.RegisterHandler(EventTypePredation, func(state *State, event *eventsource.Event) error {
		return applyPredation(state, event)
	})
	sm.RegisterHandler(EventTypePredatorDeath, func(state *State, event *eventsource.Event) error {
		return applyPredatorDeath(state, event)
	})
...
func applyPreyReproduce(state *State, event *eventsource.Event) error {
...
func applyPredation(state *State, event *eventsource.Event) error {
	// No data transformations for this transition
	_ = state
	_ = event
	return nil
...
func applyPredatorDeath(state *State, event *eventsource.Event) error {
	// No data transformations for this transition
	_ = state
	_ = event
	return nil
