...
func TestWorkflowConstants(t *testing.T) {
	...
	places := AllPlaces()
	if len(places) != 6 {
		t.Errorf("expected 6 places, got %d", len(places))
	}
...
func TestInitialPlaces(t *testing.T) {
	...
	if initial[PlaceReceived] != 1 {
		t.Errorf("expected PlaceReceived initial = 1, got %d", initial[PlaceReceived])
	}
}
...
func TestInitialState(t *testing.T) {
	agg := NewAggregate("")
	places := agg.Places()
	if places[PlaceReceived] != 1 {
		t.Errorf("expected PlaceReceived = 1, got %d", places[PlaceReceived])
	}
}
