...
func NewAggregate(id string) *Aggregate {
	...
	sm.AddTransition(eventsource.Transition{
		ID:        TransitionTurnOff,
		EventType: EventTypeTurnOff,
		Inputs: map[string]int{
			PlaceHeaterOn: 1,
		},
		Outputs: map[string]int{
	...
	sm.RegisterHandler(EventTypeHeat, func(state *State, event *eventsource.Event) error {
		return applyHeat(state, event)
	})
	sm.RegisterHandler(EventTypeCool, func(state *State, event *eventsource.Event) error {
		return applyCool(state, event)
	})
	sm.RegisterHandler(EventTypeTurnOn, func(state *State, event *eventsource.Event) error {
		return applyTurnOn(state, event)
	})
	sm.RegisterHandler(EventTypeTurnOff, func(state *State, event *eventsource.Event) error {
		return applyTurnOff(state, event)
	})
...
func applyHeat(state *State, event *eventsource.Event) error {
...
func applyCool(state *State, event *eventsource.Event) error {
	// No data transformations for this transition
	_ = state
	_ = event
	return nil
...
func applyTurnOn(state *State, event *eventsource.Event) error {
	// No data transformations for this transition
	_ = state
	_ = event
	return nil
...
func applyTurnOff(state *State, event *eventsource.Event) error {
	// No data transformations for this transition
	_ = state
	_ = event
	return nil
