(function(){const t=document.createElement("link").relList;if(t&&t.supports&&t.supports("modulepreload"))return;for(const o of document.querySelectorAll('link[rel="modulepreload"]'))a(o);new MutationObserver(o=>{for(const r of o)if(r.type==="childList")for(const l of r.addedNodes)l.tagName==="LINK"&&l.rel==="modulepreload"&&a(l)}).observe(document,{childList:!0,subtree:!0});function n(o){const r={};return o.integrity&&(r.integrity=o.integrity),o.referrerPolicy&&(r.referrerPolicy=o.referrerPolicy),o.crossOrigin==="use-credentials"?r.credentials="include":o.crossOrigin==="anonymous"?r.credentials="omit":r.credentials="same-origin",r}function a(o){if(o.ep)return;o.ep=!0;const r=n(o);fetch(o.href,r)}})();const A=[{path:"/",component:"List",title:"job-application"},{path:"/job-application",component:"List",title:"job-application"},{path:"/job-application/new",component:"Form",title:"New job-application"},{path:"/job-application/:id",component:"Detail",title:"job-application Detail"},{path:"/admin",component:"AdminDashboard",title:"Admin Dashboard"},{path:"/admin/instances",component:"AdminInstances",title:"Instances"},{path:"/admin/instances/:id",component:"AdminInstance",title:"Instance Detail"}];let v=null,b={};function j(e){e=e||"/",e!=="/"&&e.endsWith("/")&&(e=e.slice(0,-1));for(const t of A){const n={};let a=t.path.replace(/[.*+?^${}()|[\]\\]/g,"\\$&");a=a.replace(/:[^/]+/g,"([^/]+)");const o=new RegExp(`^${a}$`),r=e.match(o);if(r)return(t.path.match(/:[^/]+/g)||[]).map(h=>h.slice(1)).forEach((h,J)=>{n[h]=decodeURIComponent(r[J+1])}),{route:t,params:n}}return null}function c(e,t={}){e.startsWith("/")||(e="/"+e);const n=j(e);if(!n){console.warn(`No route found for path: ${e}, falling back to list`),e="/job-application";const a=j(e);a&&(v=a.route,b=a.params,window.history.pushState(t,"",e),I());return}if(n.route.roles&&n.route.roles.length>0){const a=W();if(!a||!U(a,n.route.roles)){console.warn("Access denied:",e),c("/job-application");return}}v=n.route,b=n.params,window.history.pushState(t,"",e),I()}window.addEventListener("popstate",()=>{const e=window.location.pathname,t=j(e);t?(v=t.route,b=t.params,I()):c("/job-application")});function W(){const e=localStorage.getItem("auth");if(e)try{return JSON.parse(e).user}catch{return null}return null}function U(e,t){return!e||!e.roles?!1:t.some(n=>e.roles.includes(n))}function I(){window.dispatchEvent(new CustomEvent("route-change",{detail:{route:v,params:b}}))}function z(){return b}function E(){return v}function V(){const e=window.location.pathname,t=j(e);t?(v=t.route,b=t.params):(v=A.find(n=>n.path==="/job-application")||A[0],b={})}const C={brand:"job-application",items:[{label:"job-application",path:"/job-application",icon:""},{label:"New",path:"/job-application/new",icon:"+"},{label:"Admin",path:"/admin",icon:""}]};let s=null,N=!1;async function F(){if(!N){N=!0;try{const e={},t=q();t&&(e.Authorization=`Bearer ${t}`);const n=await fetch("/api/navigation",{headers:e});n.ok?s=await n.json():s=C}catch{s=C}finally{N=!1}}}async function D(){s||await F();const e=window.location.pathname,t=K(),n=(s==null?void 0:s.items)||C.items;return`
  ...
  `}window.handleNavClick=function(e,t){e.preventDefault(),c(t)};window.handleLogout=async function(){try{const e=q();e&&await fetch("/auth/logout",{method:"POST",headers:{Authorization:`Bearer ${e}`}})}catch(e){console.error("Logout error:",e)}localStorage.removeItem("auth"),s=null,window.dispatchEvent(new CustomEvent("auth-change")),await R(),c("/job-application")};function K(){const e=localStorage.getItem("auth");if(e)try{return JSON.parse(e).user}catch{return null}return null}function q(){const e=localStorage.getItem("auth");if(e)try{return JSON.parse(e).token}catch{return null}return null}async function R(){s=null,await F();const e=document.getElementById("nav");e&&(e.innerHTML=await D())}window.addEventListener("auth-change",async()=>{await R()});window.addEventListener("route-change",()=>{const e=window.location.pathname;document.querySelectorAll(".nav-menu li").forEach(t=>{t.classList.remove("active")}),document.querySelectorAll(".nav-menu a").forEach(t=>{const n=t.getAttribute("href");(n===e||n!=="/"&&e.startsWith(n))&&t.parentElement.classList.add("active")})});let P=[];async function G(){try{const e=await fetch("/api/views");return e.ok?(P=await e.json(),P):(console.warn("Failed to load view definitions, using defaults"),[])}catch(e){return console.error("Error loading views:",e),[]}}const u="";let p=null,g=null,w=[],i=null;function Q(){const e=localStorage.getItem("auth");if(e)try{const t=JSON.parse(e);if(t.expires_at&&new Date(t.expires_at)>new Date)return g=t.token,p=t.user,!0;localStorage.removeItem("auth")}catch{localStorage.removeItem("auth")}return!1}function x(e){localStorage.setItem("auth",JSON.stringify(e)),g=e.token,p=e.user,window.dispatchEvent(new CustomEvent("auth-change"))}function S(){localStorage.removeItem("auth"),g=null,p=null,window.dispatchEvent(new CustomEvent("auth-change"))}function f(){const e={"Content-Type":"application/json"};return g&&(e.Authorization=`Bearer ${g}`),e}async function y(e){if(e.status===401)throw S(),$("Session expired. Please log in again."),new Error("Unauthorized");if(!e.ok){const t=await e.json().catch(()=>({}));throw new Error(t.message||e.statusText)}return e.json()}const d={async getMe(){const e=await fetch(`${u}/auth/me`,{headers:f()});return y(e)},async logout(){await fetch(`${u}/auth/logout`,{method:"POST",headers:f()}),S()},async listInstances(){const e=await fetch(`${u}/admin/instances`,{headers:f()});return y(e)},async getInstance(e){const t=await fetch(`${u}/api/jobapplication/${e}`,{headers:f()});return y(t)},async createInstance(e={}){const t=await fetch(`${u}/api/jobapplication`,{method:"POST",headers:f(),body:JSON.stringify(e)});return y(t)},async executeTransition(e,t,n={}){const a=await fetch(`${u}/api/${e}`,{method:"POST",headers:f(),body:JSON.stringify({aggregate_id:t,data:n})});return y(a)}};window.api=d;window.setAuthToken=function(e){g=e};window.saveAuth=x;window.clearAuth=S;function $(e){const t=document.getElementById("app"),n=t.querySelector(".alert-error");n&&n.remove();const a=document.createElement("div");a.className="alert alert-error",a.textContent=e,t.insertBefore(a,t.firstChild),setTimeout(()=>a.remove(),5e3)}function H(e){const t=document.getElementById("app"),n=t.querySelector(".alert-success");n&&n.remove();const a=document.createElement("div");a.className="alert alert-success",a.textContent=e,t.insertBefore(a,t.firstChild),setTimeout(()=>a.remove(),3e3)}function L(e){if(!e)return"unknown";for(const[t,n]of Object.entries(e))if(n>0)return t;return"unknown"}function O(e){return`<span class="badge ${`badge-${e.toLowerCase().replace(/_/g,"-")}`}">${e.replace(/_/g," ")}</span>`}async function T(){const e=document.getElementById("app");e.innerHTML=`
    <div class="page">
  ...
  `;try{w=(await d.listInstances()).instances||[],X()}catch{document.getElementById("instances-list").innerHTML=`
    ...
    `;return}e.innerHTML=w.map(t=>{const n=L(t.state||t.places);return`
  ...
  `;try{const[t,n]=await Promise.all([fetch(`${u}/admin/stats`,{headers:f()}).then(o=>o.json()).catch(()=>null),d.listInstances()]);t?document.getElementById("admin-stats").innerHTML=`
