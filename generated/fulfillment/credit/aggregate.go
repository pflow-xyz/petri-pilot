...
// State holds the aggregate state for credit.
type State struct {
...
// Aggregate wraps a StateMachine with the credit state.
...
func NewAggregate(id string) *Aggregate {
	...
	sm.AddTransition(eventsource.Transition{
		ID:        TransitionClearCredit,
		EventType: EventTypeClearCredit,
	...
	sm.AddTransition(eventsource.Transition{
		ID:        TransitionRevokeCredit,
		EventType: EventTypeRevokeCredit,
		Inputs: map[string]int{
			PlaceCleared: 1,
		},
		Outputs: map[string]int{
	...
	sm.RegisterHandler(EventTypeClearCredit, func(state *State, event *eventsource.Event) error {
		return applyClearCredit(state, event)
	})
	sm.RegisterHandler(EventTypeRevokeCredit, func(state *State, event *eventsource.Event) error {
		return applyRevokeCredit(state, event)
	})
...
func applyClearCredit(state *State, event *eventsource.Event) error {
...
func applyRevokeCredit(state *State, event *eventsource.Event) error {
	// No data transformations for this transition
	_ = state
	_ = event
	return nil
...
func (app *Application) Load(ctx context.Context, id string) (*Aggregate, error) {
	events, err := app.store.Read(ctx, "credit/"+id, 0)
	if err != nil {
...
func (app *Application) Execute(ctx context.Context, id, transitionID string, data any) (*Aggregate, error) {
	...
	_, err = app.store.Append(ctx, "credit/"+id, agg.Version(), []*eventsource.Event{event})
	if err != nil {
...
func (app *Application) TruncateTo(ctx context.Context, id string, targetVersion int) (*Aggregate, error) {
	...
	if len(eventsToKeep) > 0 {
		if _, err := app.store.Append(ctx, "credit/"+id, -1, eventsToKeep); err != nil {
