...
// RemoveSessionIfSame removes a session only if it's the exact same instance.
// This prevents race conditions where an old connection's cleanup removes a new session.
func (b *DebugBroker) RemoveSessionIfSame(session *DebugSession) {
	b.sessionsLock.Lock()
	defer b.sessionsLock.Unlock()
	if current, ok := b.sessions[session.ID]; ok && current == session {
		close(session.send)
...
func HandleDebugWebSocket(broker *DebugBroker) http.HandlerFunc {
	return func(w http.ResponseWriter, r *http.Request) {
		...
		// Allow client to specify session ID for reconnection
		sessionID := r.URL.Query().Get("session")
		if sessionID == "" {
			sessionID = broker.GenerateSessionID()
		} else if old := broker.GetSession(sessionID); old != nil {
			// Close old connection to allow reconnect
