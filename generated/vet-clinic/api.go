...
func BuildRouter(app *Application, middleware *Middleware, sessions SessionStore, navigation *Navigation, debugBroker *DebugBroker) http.Handler {
	...
	r.POST("/api/simulation/start", "Start simulation", HandleStartSimulation(app))
	r.POST("/api/simulation/stop", "Stop simulation", HandleStopSimulation(app))
	r.GET("/api/simulation/status", "Get simulation status", HandleGetSimulationStatus(app))
	...
	r.Transition("schedule", "/api/schedule", "", middleware.RequirePermission("schedule")(HandleSchedule(app)))
	r.Transition("check_in", "/api/check_in", "", middleware.RequirePermission("check_in")(HandleCheckIn(app)))
	r.Transition("start_exam", "/api/start_exam", "", middleware.RequirePermission("start_exam")(HandleStartExam(app)))
	r.Transition("complete", "/api/complete", "", middleware.RequirePermission("complete")(HandleComplete(app)))
	r.Transition("checkout", "/api/checkout", "", middleware.RequirePermission("checkout")(HandleCheckout(app)))
	r.Transition("cancel_requested", "/api/cancel_requested", "", middleware.RequirePermission("cancel_requested")(HandleCancelRequested(app)))
	r.Transition("cancel_scheduled", "/api/cancel_scheduled", "", middleware.RequirePermission("cancel_scheduled")(HandleCancelScheduled(app)))
	r.Transition("mark_no_show", "/api/mark_no_show", "", middleware.RequirePermission("mark_no_show")(HandleMarkNoShow(app)))
	r.Transition("send_to_lab", "/api/send_to_lab", "", middleware.RequirePermission("send_to_lab")(HandleSendToLab(app)))
	r.Transition("receive_results", "/api/receive_results", "", middleware.RequirePermission("receive_results")(HandleReceiveResults(app)))
	r.Transition("request_auth", "/api/request_auth", "", middleware.RequirePermission("request_auth")(HandleRequestAuth(app)))
	r.Transition("authorize", "/api/authorize", "", middleware.RequirePermission("authorize")(HandleAuthorize(app)))
	r.Transition("refer_out", "/api/refer_out", "", middleware.RequirePermission("refer_out")(HandleReferOut(app)))
...
// HandleSchedule handles the schedule transition.
func HandleSchedule(app *Application) http.HandlerFunc {
	return func(w http.ResponseWriter, r *http.Request) {
		ctx := r.Context()
		...
		agg, err := app.Execute(ctx, req.AggregateID, TransitionSchedule, req.Data)
		if err != nil {
...
// HandleCheckIn handles the check_in transition.
func HandleCheckIn(app *Application) http.HandlerFunc {
	return func(w http.ResponseWriter, r *http.Request) {
		ctx := r.Context()
		...
		agg, err := app.Execute(ctx, req.AggregateID, TransitionCheckIn, req.Data)
		if err != nil {
			api.Error(w, http.StatusConflict, "TRANSITION_FAILED", err.Error())
			return
		}
...
// HandleStartExam handles the start_exam transition.
func HandleStartExam(app *Application) http.HandlerFunc {
	return func(w http.ResponseWriter, r *http.Request) {
		ctx := r.Context()
		...
		agg, err := app.Execute(ctx, req.AggregateID, TransitionStartExam, req.Data)
		if err != nil {
			api.Error(w, http.StatusConflict, "TRANSITION_FAILED", err.Error())
			return
		}
...
// HandleCheckout handles the checkout transition.
func HandleCheckout(app *Application) http.HandlerFunc {
	return func(w http.ResponseWriter, r *http.Request) {
		ctx := r.Context()
		...
		agg, err := app.Execute(ctx, req.AggregateID, TransitionCheckout, req.Data)
		if err != nil {
			api.Error(w, http.StatusConflict, "TRANSITION_FAILED", err.Error())
			return
		}
...
// HandleCancelRequested handles the cancel_requested transition.
func HandleCancelRequested(app *Application) http.HandlerFunc {
	return func(w http.ResponseWriter, r *http.Request) {
		ctx := r.Context()
		...
		agg, err := app.Execute(ctx, req.AggregateID, TransitionCancelRequested, req.Data)
		if err != nil {
			api.Error(w, http.StatusConflict, "TRANSITION_FAILED", err.Error())
			return
		}
...
// HandleCancelScheduled handles the cancel_scheduled transition.
func HandleCancelScheduled(app *Application) http.HandlerFunc {
	return func(w http.ResponseWriter, r *http.Request) {
		ctx := r.Context()
		...
		agg, err := app.Execute(ctx, req.AggregateID, TransitionCancelScheduled, req.Data)
		if err != nil {
			api.Error(w, http.StatusConflict, "TRANSITION_FAILED", err.Error())
			return
		}
...
// HandleMarkNoShow handles the mark_no_show transition.
func HandleMarkNoShow(app *Application) http.HandlerFunc {
	return func(w http.ResponseWriter, r *http.Request) {
		ctx := r.Context()
		...
		agg, err := app.Execute(ctx, req.AggregateID, TransitionMarkNoShow, req.Data)
		if err != nil {
			api.Error(w, http.StatusConflict, "TRANSITION_FAILED", err.Error())
			return
		}
...
// HandleSendToLab handles the send_to_lab transition.
func HandleSendToLab(app *Application) http.HandlerFunc {
	return func(w http.ResponseWriter, r *http.Request) {
		ctx := r.Context()
		...
		agg, err := app.Execute(ctx, req.AggregateID, TransitionSendToLab, req.Data)
		if err != nil {
			api.Error(w, http.StatusConflict, "TRANSITION_FAILED", err.Error())
			return
		}
...
// HandleReceiveResults handles the receive_results transition.
func HandleReceiveResults(app *Application) http.HandlerFunc {
	return func(w http.ResponseWriter, r *http.Request) {
		ctx := r.Context()
		...
		agg, err := app.Execute(ctx, req.AggregateID, TransitionReceiveResults, req.Data)
		if err != nil {
			api.Error(w, http.StatusConflict, "TRANSITION_FAILED", err.Error())
			return
		}
...
// HandleRequestAuth handles the request_auth transition.
func HandleRequestAuth(app *Application) http.HandlerFunc {
	return func(w http.ResponseWriter, r *http.Request) {
		ctx := r.Context()
		...
		agg, err := app.Execute(ctx, req.AggregateID, TransitionRequestAuth, req.Data)
		if err != nil {
			api.Error(w, http.StatusConflict, "TRANSITION_FAILED", err.Error())
			return
		}
...
// HandleAuthorize handles the authorize transition.
func HandleAuthorize(app *Application) http.HandlerFunc {
	return func(w http.ResponseWriter, r *http.Request) {
		ctx := r.Context()
		...
		agg, err := app.Execute(ctx, req.AggregateID, TransitionAuthorize, req.Data)
		if err != nil {
			api.Error(w, http.StatusConflict, "TRANSITION_FAILED", err.Error())
			return
		}
...
// HandleReferOut handles the refer_out transition.
func HandleReferOut(app *Application) http.HandlerFunc {
	return func(w http.ResponseWriter, r *http.Request) {
		ctx := r.Context()
		...
		agg, err := app.Execute(ctx, req.AggregateID, TransitionReferOut, req.Data)
		if err != nil {
			api.Error(w, http.StatusConflict, "TRANSITION_FAILED", err.Error())
			return
		}
