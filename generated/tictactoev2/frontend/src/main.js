...
const api = {
  ...
  async getInstance(id) {
    const response = await fetch(`${API_BASE}/api/tictactoev2/${id}`, { headers: getHeaders() })
    return handleResponse(response)
  },

  async createInstance(data = {}) {
    const response = await fetch(`${API_BASE}/api/tictactoev2`, {
      method: 'POST',
      headers: getHeaders(),
...
window.pilot = {
  ...
  async getEvents() {
    if (!currentInstance) throw new Error('No current instance')
    const response = await fetch(`${API_BASE}/api/tictactoev2/${currentInstance.id}/events`, {
      headers: getHeaders()
    })
