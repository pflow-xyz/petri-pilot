...
func TestWorkflowConstants(t *testing.T) {
	...
	places := AllPlaces()
	if len(places) != 10 {
		t.Errorf("expected 10 places, got %d", len(places))
	}

	// Verify transition constants
	transitions := AllTransitions()
	if len(transitions) != 11 {
		t.Errorf("expected 11 transitions, got %d", len(transitions))
	}
}
...
func TestInitialPlaces(t *testing.T) {
	...
	if initial[PlaceApplied] != 1 {
		t.Errorf("expected PlaceApplied initial = 1, got %d", initial[PlaceApplied])
	}
}
...
func TestInitialState(t *testing.T) {
	...
	if places[PlaceApplied] != 1 {
		t.Errorf("expected PlaceApplied = 1, got %d", places[PlaceApplied])
	}
}
