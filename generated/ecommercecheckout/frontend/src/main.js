...
function hasCustomDashboard() {
  return customElements.get('pf-ecommerce-checkout-dashboard') !== undefined
}
...
const TRANSITION_DEFS = [
  ...
  },
  {
    id: 'payment_fail_1',
    name: 'Payment Fail 1',
    description: "First payment attempt failed",
    fields: [
    ]
  },
  {
    ...
    description: "Retry payment (attempt 2)",
    fields: [
    ]
  },
  {
    id: 'payment_fail_2',
    name: 'Payment Fail 2',
    description: "Second payment attempt failed",
    fields: [
    ]
  },
  {
    ...
    description: "Retry payment (attempt 3)",
    fields: [
    ]
  },
  {
    id: 'payment_fail_3',
    name: 'Payment Fail 3',
    description: "Third payment attempt failed",
    fields: [
    ]
  },
  {
    id: 'cancel_order',
    name: 'Cancel Order',
    description: "Cancel order after max retries",
    fields: [
    ]
  },
  {
...
const api = {
  ...
  async getInstance(id) {
    const response = await fetch(`${API_BASE}/api/ecommercecheckout/${id}`, { headers: getHeaders() })
    return handleResponse(response)
  },

  async createInstance(data = {}) {
    const response = await fetch(`${API_BASE}/api/ecommercecheckout`, {
      method: 'POST',
      headers: getHeaders(),
...
async function renderListPage() {
  ...
  if (hasCustomDashboard()) {
    app.innerHTML = `<pf-ecommerce-checkout-dashboard></pf-ecommerce-checkout-dashboard>`
    return
  }
...
window.pilot = {
  ...
  async getEvents() {
    if (!currentInstance) throw new Error('No current instance')
    const response = await fetch(`${API_BASE}/api/ecommercecheckout/${currentInstance.id}/events`, {
      headers: getHeaders()
    })
  ...
  getPlaces() {
    return [
      { id: 'cart', name: 'Cart', initial: 1 },
      { id: 'checkout_started', name: 'CheckoutStarted', initial: 0 },
      { id: 'payment_pending', name: 'PaymentPending', initial: 0 },
      { id: 'payment_processing', name: 'PaymentProcessing', initial: 0 },
      { id: 'retry_1', name: 'Retry1', initial: 0 },
      { id: 'retry_2', name: 'Retry2', initial: 0 },
      { id: 'retry_3', name: 'Retry3', initial: 0 },
      { id: 'paid', name: 'Paid', initial: 0 },
      { id: 'cancelled', name: 'Cancelled', initial: 0 },
      { id: 'fulfilled', name: 'Fulfilled', initial: 0 },
    ]
  },
