...
// State holds the aggregate state for order.
type State struct {
...
// Aggregate wraps a StateMachine with the order state.
...
func NewAggregate(id string) *Aggregate {
	...
	sm.AddTransition(eventsource.Transition{
		ID:        TransitionPlaceOrder,
		EventType: EventTypePlaceOrder,
	...
	sm.RegisterHandler(EventTypePlaceOrder, func(state *State, event *eventsource.Event) error {
		return applyPlaceOrder(state, event)
	})
...
var crossEntityCommands = map[string]string{
	TransitionPlaceOrder: "order_reserves_stock", // fused
	TransitionShip:       "order/ship",           // guarded
}
...
func applyPlaceOrder(state *State, event *eventsource.Event) error {
...
func (app *Application) Load(ctx context.Context, id string) (*Aggregate, error) {
	events, err := app.store.Read(ctx, "order/"+id, 0)
	if err != nil {
...
func (app *Application) Execute(ctx context.Context, id, transitionID string, data any) (*Aggregate, error) {
	...
	_, err = app.store.Append(ctx, "order/"+id, agg.Version(), []*eventsource.Event{event})
	if err != nil {
...
func (app *Application) TruncateTo(ctx context.Context, id string, targetVersion int) (*Aggregate, error) {
	...
	if len(eventsToKeep) > 0 {
		if _, err := app.store.Append(ctx, "order/"+id, -1, eventsToKeep); err != nil {
