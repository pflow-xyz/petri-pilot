...
function hasCustomDashboard() {
  return customElements.get('pf-task-manager-dashboard') !== undefined
}
...
const TRANSITION_DEFS = [
  ...
  },
  {
    id: 'reject',
    name: 'Reject',
    description: "Reject task and send back",
    fields: [
    ]
  },
]
...
const api = {
  ...
  async getInstance(id) {
    const response = await fetch(`${API_BASE}/api/taskmanager/${id}`, { headers: getHeaders() })
    return handleResponse(response)
  },

  async createInstance(data = {}) {
    const response = await fetch(`${API_BASE}/api/taskmanager`, {
      method: 'POST',
      headers: getHeaders(),
...
async function renderListPage() {
  ...
  if (hasCustomDashboard()) {
    app.innerHTML = `<pf-task-manager-dashboard></pf-task-manager-dashboard>`
    return
  }
...
window.pilot = {
  ...
  async getEvents() {
    if (!currentInstance) throw new Error('No current instance')
    const response = await fetch(`${API_BASE}/api/taskmanager/${currentInstance.id}/events`, {
      headers: getHeaders()
    })
  ...
  getPlaces() {
    return [
      { id: 'pending', name: 'Pending', initial: 1 },
      { id: 'in_progress', name: 'InProgress', initial: 0 },
      { id: 'review', name: 'Review', initial: 0 },
