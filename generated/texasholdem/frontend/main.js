...
let gameState = {
  ...
  dealer: 0,
  sbPlayer: 1,  // Small blind player (updated in postBlinds)
  bbPlayer: 2,  // Big blind player (updated in postBlinds)
  currentPlayer: 0,
  // Blind tracking
  blindLevel: 0,
  handsPlayed: 0,
  ...
  eventSeq: 0     // Sequence number within current hand
}

// Helper to add event with hand#-seq
function addEvent(type) {
  gameState.eventSeq++
  gameState.events.push({
    type,
    hand: gameState.handsPlayed,
    seq: gameState.eventSeq
  })
}
...
document.addEventListener('DOMContentLoaded', () => {
  ...
  document.getElementById('play-tournament-btn').addEventListener('click', playTournament)
...
/**
 * Play a full tournament - single button to start everything
 */
async function playTournament() {
  ...
  // Create first game and start
  await createNewGameAndStartHand()

  // Start auto-play
  autoPlayActive = true
  document.getElementById('play-tournament-btn').style.display = 'none'
...
/**
 * Create a new game and immediately start a hand (used by tournament mode)
 */
async function createNewGameAndStartHand() {
  try {
    // Reset event sequence for this hand
    gameState.eventSeq = 0

    // Create new aggregate
    const createResponse = await fetch(`${getApiBase()}/api/texasholdem`, {
      method: 'POST',
    ...
    gameState.id = createData.aggregate_id
    gameState.version = createData.version
    gameState.places = createData.state
    gameState.enabled = createData.enabled_transitions || []

    // Start the hand
    const startResponse = await fetch(`${getApiBase()}/api/start_hand`, {
      method: 'POST',
      headers: { 'Content-Type': 'application/json' },
    ...
    if (!startResponse.ok) {
      throw new Error(`Failed to start hand: ${startResponse.status}`)
    }

    const startData = await startResponse.json()
    updateGameState(startData)

    // Reset local state for this hand
    gameState.players.forEach(p => {
      p.cards = []
      p.bet = 0
      // Players with no chips are eliminated (marked as folded)
      p.folded = (p.chips <= 0)
    })
    ...
    console.error('Failed to create game and start hand:', err)
...
/**
 * Create a new game instance (legacy - kept for compatibility)
 */
...
async function startHand() {
  try {
    ...
    // Create a new shuffled deck and deal hole cards to players with chips
    createDeck()
    for (let i = 0; i < 5; i++) {
      if (gameState.players[i].chips > 0) {
        gameState.players[i].cards = dealCards(2)
        ...
        gameState.players[i].folded = true  // Mark eliminated players as folded
      }
    }
...
/**
 * Find the next active player (with chips) after the given position
 */
function findNextActivePlayer(startPosition) {
  let pos = (startPosition + 1) % 5
  let iterations = 0
  while (gameState.players[pos].chips <= 0 && iterations < 5) {
    pos = (pos + 1) % 5
    iterations++
  }
  return pos
 ...
 */
function postBlinds() {
  // Small blind is first active player after dealer
  const sbPlayer = findNextActivePlayer(gameState.dealer)
  // Big blind is first active player after small blind
  const bbPlayer = findNextActivePlayer(sbPlayer)
  ...
  // Store blind positions in game state for rendering
  gameState.sbPlayer = sbPlayer
  gameState.bbPlayer = bbPlayer

  // Log blinds
  addEvent(`${gameState.players[sbPlayer].name} posts SB $${sbAmount}`)
  addEvent(`${gameState.players[bbPlayer].name} posts BB $${bbAmount}`)
...
function updateBlindLevel() {
  ...
  if (newLevel > gameState.blindLevel) {
    ...
    addEvent(`Blinds increased to $${gameState.smallBlind}/$${gameState.bigBlind}`)
  }
}
...
function parseGameState() {
  ...
  // Update player folded status - check both backend state AND chips (eliminated players)
  for (let i = 0; i < 5; i++) {
    gameState.players[i].folded = places[`p${i}_folded`] > 0 || gameState.players[i].chips <= 0
  }
}
...
function renderPokerTable() {
  ...
  gameState.players.forEach((player, i) => {
    ...
    // Update blind markers (using stored positions that skip eliminated players)
    ...
    if (i === gameState.sbPlayer && gameState.currentRound !== 'waiting') {
      blindMarker.textContent = 'SB'
      blindMarker.className = 'blind-marker sb'
      blindMarker.style.display = 'inline-block'
    } else if (i === gameState.bbPlayer && gameState.currentRound !== 'waiting') {
...
async function processGameUntilPlayerTurn() {
  ...
  while (iterations < maxIterations) {
    ...
    // If Player 0 is eliminated, auto-skip them
    if (gameState.currentPlayer === 0 && (gameState.players[0].chips <= 0 || gameState.players[0].folded)) {
      const skipped = await processPlayer0IfEliminated()
      if (skipped) {
        await delay(200)
        continue
      }
    ...
    // If it's Player 0's turn and they have chips, stop and let human play
...
/**
 * Check if a player has an available action (not including skip)
 */
function hasPlayerAction(playerIndex) {
  // Eliminated players have no regular actions (may have skip)
...
/**
 * Process player 0's turn if they're eliminated (use skip)
 */
async function processPlayer0IfEliminated() {
  const enabledSet = new Set(gameState.enabled || [])
  const player = gameState.players[0]

  if (player.chips <= 0 || player.folded) {
    if (enabledSet.has('p0_skip')) {
      console.log('Player 0 eliminated/all-in - using skip transition')
      await executeTransition('p0_skip', {})
      return true
    }
  }
  return false
 ...
 * Eliminated/all-in players use skip transitions to advance the game
 */
async function processAIPlayerTurn() {
  ...
  for (let i = 1; i <= 4; i++) {
    ...
    // Eliminated/all-in players use skip transition to advance
    if (gameState.players[i].chips <= 0 || gameState.players[i].folded) {
      if (enabledSet.has(prefix + 'skip')) {
        console.log(`Player ${i} eliminated/all-in - using skip transition`)
        await executeTransition(prefix + 'skip', {})
        return true
      }
      // Fallback to fold if skip not available (legacy support)
      if (enabledSet.has(prefix + 'fold')) {
        console.log(`Player ${i} eliminated - auto-folding (fallback)`)
...
function renderEventHistory() {
  ...
  listEl.innerHTML = gameState.events.slice(-10).reverse().map(event => {
    ...
    // Show hand#-seq instead of timestamp
    const handSeq = event.hand !== undefined ? `H${event.hand}-${event.seq}` : ''

    return `
      <div class="event-item">
        <strong>${displayType}</strong>
        ${handSeq ? `<br><small>${handSeq}</small>` : ''}
      </div>
    `
  }).join('')
}
...
function stopAutoPlay() {
  ...
  document.getElementById('play-tournament-btn').style.display = 'inline-block'
...
async function refreshState() {
  ...
  try {
    const response = await fetch(`${getApiBase()}/api/texasholdem/${gameState.id}`)
    if (response.ok) {
      const data = await response.json()
      if (data.state) gameState.places = data.state
      if (data.enabled_transitions) gameState.enabled = data.enabled_transitions
      if (data.enabled) gameState.enabled = data.enabled
      parseGameState()
      console.log('State refreshed, enabled:', gameState.enabled)
    }
  } catch (err) {
    console.log('Failed to refresh state:', err)
  }
}
...
async function autoPlayLoop() {
  ...
  while (autoPlayActive && gameState.currentRound !== 'complete' && gameState.currentRound !== 'waiting') {
    ...
    // If too many failures, try to refresh state and retry
    if (consecutiveFailures > 5) {
      console.log('Too many failures, refreshing state...')
      await refreshState()
    ...
    for (let p = 0; p < 5; p++) {
      ...
      if (player.chips <= 0 || player.folded) {
        if (enabledSet.has(prefix + 'skip')) {
          console.log(`Player ${p} eliminated/all-in - using skip in main loop`)
          await executeTransition(prefix + 'skip', {})
          actionTaken = true
          consecutiveFailures = 0
          break
        ...
        // Fallback to fold if skip not available (legacy support)
        if (enabledSet.has(prefix + 'fold')) {
          console.log(`Player ${p} eliminated - auto-folding in main loop (fallback)`)
          await executeTransition(prefix + 'fold', {})
          actionTaken = true
          consecutiveFailures = 0
          break
    ...
    if (!actionTaken) {
      ...
      // Emergency fallback: if player actions are failing, try dealer actions regardless
      if (consecutiveFailures >= 3) {
        console.log('Trying dealer actions as emergency fallback...')
        for (const dealerAction of ['end_betting_round', 'go_showdown', 'determine_winner', 'end_hand', 'deal_flop', 'deal_turn', 'deal_river']) {
          try {
            await executeTransition(dealerAction, {})
            console.log(`Emergency ${dealerAction} succeeded`)
            consecutiveFailures = 0
            actionTaken = true
            break
          } catch (err) {
 ...
 * Creates a new aggregate since each hand is a separate game instance
 */
async function startNextHand() {
  try {
    ...
    // Create a new aggregate for this hand
    await createNewGameAndStartHand()
