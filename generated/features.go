...
func NewCommentStore(db *sql.DB, maxLength int) *CommentStore {
	return &CommentStore{db: db, maxLength: maxLength}
}
...
func NewTagStore(db *sql.DB, predefined []string, freeForm bool, maxTags int, colors bool) *TagStore {
	return &TagStore{db: db, predefined: predefined, freeForm: freeForm, maxTags: maxTags, colors: colors}
}
...
func NewActivityStore(db *sql.DB, maxItems int) *ActivityStore {
	return &ActivityStore{db: db, maxItems: maxItems}
}
