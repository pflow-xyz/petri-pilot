...
const eventsAPI = {
  ...
  async getEvents(aggregateId, fromVersion = 0) {
    const response = await fetch(`${API_BASE}/api/ecommerce-checkout/${aggregateId}/events?from=${fromVersion}`, {
      headers: getHeaders(),
    })
  ...
  async getStateAtVersion(aggregateId, version) {
    const response = await fetch(`${API_BASE}/api/ecommerce-checkout/${aggregateId}/at/${version}`, {
      headers: getHeaders(),
    })
    return handleResponse(response)
  },
  ...
  async createSnapshot(aggregateId) {
    const response = await fetch(`${API_BASE}/api/ecommerce-checkout/${aggregateId}/snapshot`, {
      method: 'POST',
      headers: getHeaders(),
    })
    return handleResponse(response)
  ...
  async replayFromSnapshot(aggregateId) {
    const response = await fetch(`${API_BASE}/api/ecommerce-checkout/${aggregateId}/replay`, {
      method: 'POST',
      headers: getHeaders(),
    })
    return handleResponse(response)
...
async function createSnapshot() {
  ...
  try {
    ...
    btn.textContent = '✅ Snapshot created'
    setTimeout(() => {
