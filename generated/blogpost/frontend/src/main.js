...
function hasCustomDashboard() {
  return customElements.get('pf-blog-post-dashboard') !== undefined
}
...
const TRANSITION_DEFS = [
  {
    ...
    fields: [
      { name: 'title', label: 'Title', type: 'text', required: true, autoFill: '', placeholder: 'Post title', defaultValue: '' },
      { name: 'content', label: 'Content', type: 'textarea', required: true, autoFill: '', placeholder: 'Write your post...', defaultValue: '' },
      { name: 'tags', label: 'Tags', type: 'text', required: false, autoFill: '', placeholder: 'Comma-separated tags', defaultValue: '' },
    ]
  },
  {
    id: 'update',
    ...
    fields: [
      { name: 'title', label: 'Title', type: 'text', required: false, autoFill: '', placeholder: 'Post title', defaultValue: '' },
      { name: 'content', label: 'Content', type: 'textarea', required: false, autoFill: '', placeholder: 'Write your post...', defaultValue: '' },
      { name: 'tags', label: 'Tags', type: 'text', required: false, autoFill: '', placeholder: 'Comma-separated tags', defaultValue: '' },
    ]
  },
  {
  ...
  },
  {
    ...
    fields: [
      { name: 'reason', label: 'Reason', type: 'textarea', required: false, autoFill: '', placeholder: 'Why is this being rejected?', defaultValue: '' },
    ]
  },
  {
    id: 'unpublish',
    name: 'Unpublish',
    description: "Take down a published post",
    fields: [
    ]
  },
  {
    id: 'restore',
    name: 'Restore',
    description: "Restore archived post to draft",
    fields: [
    ]
  },
]
...
const api = {
  ...
  async getInstance(id) {
    const response = await fetch(`${API_BASE}/api/blogpost/${id}`, { headers: getHeaders() })
    return handleResponse(response)
  },

  async createInstance(data = {}) {
    const response = await fetch(`${API_BASE}/api/blogpost`, {
      method: 'POST',
      headers: getHeaders(),
...
async function renderListPage() {
  ...
  if (hasCustomDashboard()) {
    app.innerHTML = `<pf-blog-post-dashboard></pf-blog-post-dashboard>`
    return
  }
...
function renderInstancesList() {
  ...
  container.innerHTML = instances.map(inst => {
    ...
    return `
      <div class="entity-card" onclick="navigate('/blog-post/${inst.id}')">
        ...
        <div class="entity-actions">
          <button class="btn btn-sm btn-secondary" onclick="event.stopPropagation(); navigate('/blog-post/${inst.id}')">
            View
          </button>
        </div>
      </div>
...
window.handleSubmitCreate = async function(event) {
  ...
  try {
    ...
    navigate(`/blog-post/${result.aggregate_id || result.id}`)
  } catch (err) {
...
function handleRouteChange(event) {
  ...
  if (path === '/blog-post' || path === '/') {
    renderListPage()
  } else if (path === '/blog-post/new') {
    renderFormPage()
  } else if (path === '/blog-post/:id') {
    renderDetailPage()
  } else if (path === '/schema') {
    renderSchemaPage()
  } else if (path === '/admin') {
...
window.pilot = {
  ...
  async getEvents() {
    if (!currentInstance) throw new Error('No current instance')
    const response = await fetch(`${API_BASE}/api/blogpost/${currentInstance.id}/events`, {
      headers: getHeaders()
    })
  ...
  getPlaces() {
    return [
      ...
      { id: 'in_review', name: 'InReview', initial: 0 },
      { id: 'published', name: 'Published', initial: 0 },
      { id: 'archived', name: 'Archived', initial: 0 },
    ]
  },
