(function(){const t=document.createElement("link").relList;if(t&&t.supports&&t.supports("modulepreload"))return;for(const i of document.querySelectorAll('link[rel="modulepreload"]'))o(i);new MutationObserver(i=>{for(const s of i)if(s.type==="childList")for(const d of s.addedNodes)d.tagName==="LINK"&&d.rel==="modulepreload"&&o(d)}).observe(document,{childList:!0,subtree:!0});function n(i){const s={};return i.integrity&&(s.integrity=i.integrity),i.referrerPolicy&&(s.referrerPolicy=i.referrerPolicy),i.crossOrigin==="use-credentials"?s.credentials="include":i.crossOrigin==="anonymous"?s.credentials="omit":s.credentials="same-origin",s}function o(i){if(i.ep)return;i.ep=!0;const s=n(i);fetch(i.href,s)}})();const D=[{path:"/",component:"List",title:"order-processing"},{path:"/orderprocessing",component:"List",title:"order-processing"},{path:"/orderprocessing/new",component:"Form",title:"New order-processing"},{path:"/orderprocessing/:id",component:"Detail",title:"order-processing Detail"},{path:"/admin",component:"AdminDashboard",title:"Admin Dashboard"},{path:"/admin/instances",component:"AdminInstances",title:"Instances"},{path:"/admin/instances/:id",component:"AdminInstance",title:"Instance Detail"}];let y=null,$={};function C(e){e=e||"/",e!=="/"&&e.endsWith("/")&&(e=e.slice(0,-1));for(const t of D){const n={};let o=t.path.replace(/[.*+?^${}()|[\]\\]/g,"\\$&");o=o.replace(/:[^/]+/g,"([^/]+)");const i=new RegExp(`^${o}$`),s=e.match(i);if(s)return(t.path.match(/:[^/]+/g)||[]).map(r=>r.slice(1)).forEach((r,h)=>{n[r]=decodeURIComponent(s[h+1])}),{route:t,params:n}}return null}function u(e,t={}){e.startsWith("/")||(e="/"+e);const n=C(e);if(!n){console.warn(`No route found for path: ${e}, falling back to list`),e="/orderprocessing";const o=C(e);o&&(y=o.route,$=o.params,window.history.pushState(t,"",e),B());return}if(n.route.roles&&n.route.roles.length>0){const o=Q();if(!o||!X(o,n.route.roles)){console.warn("Access denied:",e),u("/orderprocessing");return}}y=n.route,$=n.params,window.history.pushState(t,"",e),B()}window.addEventListener("popstate",()=>{const e=window.location.pathname,t=C(e);t?(y=t.route,$=t.params,B()):u("/orderprocessing")});function Q(){const e=localStorage.getItem("auth");if(e)try{return JSON.parse(e).user}catch{return null}return null}function X(e,t){return!e||!e.roles?!1:t.some(n=>e.roles.includes(n))}function B(){window.dispatchEvent(new CustomEvent("route-change",{detail:{route:y,params:$}}))}function Z(){return $}function x(){return y}function ee(){const e=window.location.pathname,t=C(e);t?(y=t.route,$=t.params):(y=D.find(n=>n.path==="/orderprocessing")||D[0],$={})}const W=[{id:"customer",description:"End user placing orders"},{id:"fulfillment",description:"Warehouse staff who validate and ship orders"},{id:"system",description:"Automated payment processing"},{id:"admin",description:"Full access to all operations"}],N={brand:"order-processing",items:[{label:"order-processing",path:"/orderprocessing",icon:""},{label:"New",path:"/orderprocessing/new",icon:"+"},{label:"Admin",path:"/admin",icon:""}]};let l=null,F=!1;async function U(){if(!F){F=!0;try{const e={},t=G();t&&(e.Authorization=`Bearer ${t}`);const n=await fetch("/api/navigation",{headers:e});n.ok?l=await n.json():l=N}catch{l=N}finally{F=!1}}}async function K(){l||await U();const e=window.location.pathname,t=ne(),n=(l==null?void 0:l.items)||N.items;return`
  ...
  `}window.showLoginModal=function(){const e=document.getElementById("login-modal");e&&(e.style.display="flex")};window.hideLoginModal=function(){const e=document.getElementById("login-modal");e&&(e.style.display="none")};window.handleRoleLogin=async function(e){try{const t=await fetch("/api/debug/login",{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify({login:"pilot-user",roles:e})});if(!t.ok)throw new Error("Login failed");const n=await t.json();localStorage.setItem("auth",JSON.stringify(n)),hideLoginModal(),l=null,window.dispatchEvent(new CustomEvent("auth-change")),await I()}catch(t){console.error("Login error:",t),alert("Login failed. Please try again.")}};window.handleNavClick=function(e,t){e.preventDefault(),u(t)};window.handleLogout=async function(){try{const e=G();e&&await fetch("/auth/logout",{method:"POST",headers:{Authorization:`Bearer ${e}`}})}catch(e){console.error("Logout error:",e)}localStorage.removeItem("auth"),l=null,window.dispatchEvent(new CustomEvent("auth-change")),await I(),u("/orderprocessing")};function ne(){const e=localStorage.getItem("auth");if(e)try{return JSON.parse(e).user}catch{return null}return null}function G(){const e=localStorage.getItem("auth");if(e)try{return JSON.parse(e).token}catch{return null}return null}async function I(){l=null,await U();const e=document.getElementById("nav");e&&(e.innerHTML=await K())}window.addEventListener("auth-change",async()=>{await I()});window.addEventListener("route-change",()=>{const e=window.location.pathname;document.querySelectorAll(".nav-menu li").forEach(t=>{t.classList.remove("active")}),document.querySelectorAll(".nav-menu a").forEach(t=>{const n=t.getAttribute("href");(n===e||n!=="/"&&e.startsWith(n))&&t.parentElement.classList.add("active")})});let J=[];async function oe(){try{const e=await fetch("/api/views");return e.ok?(J=await e.json(),J):(console.warn("Failed to load view definitions, using defaults"),[])}catch(e){return console.error("Error loading views:",e),[]}}const f="",ae="";function se(e){return["amount","value","balance","total_supply","allowance"].some(n=>e.toLowerCase().includes(n))}let c=null,m=null,b=[],a=null;const j=[{id:"validate",name:"Validate",description:"Check order validity",fields:[]},{id:"reject",name:"Reject",description:"Mark order as invalid",fields:[]},{id:"process_payment",name:"Process Payment",description:"Charge customer payment",fields:[]},{id:"ship",name:"Ship",description:"Send order to shipping",fields:[]},{id:"confirm",name:"Confirm",description:"Mark order as complete",fields:[]}];function ie(){const e=localStorage.getItem("auth");if(e)try{const t=JSON.parse(e);if(t.expires_at&&new Date(t.expires_at)>new Date)return m=t.token,c=t.user,!0;localStorage.removeItem("auth")}catch{localStorage.removeItem("auth")}return!1}function q(e){localStorage.setItem("auth",JSON.stringify(e)),m=e.token,c=e.user,window.dispatchEvent(new CustomEvent("auth-change"))}function A(){localStorage.removeItem("auth"),m=null,c=null,window.dispatchEvent(new CustomEvent("auth-change"))}function re(){const e=localStorage.getItem("auth");if(e)try{const t=JSON.parse(e);return m=t.token,c=t.user,!0}catch{return!1}return m=null,c=null,!1}window.addEventListener("auth-change",()=>{re()});function w(){const e={"Content-Type":"application/json"};return m&&(e.Authorization=`Bearer ${m}`),e}async function E(e){if(e.status===401)throw A(),S("Session expired. Please log in again."),new Error("Unauthorized");if(!e.ok){const t=await e.json().catch(()=>({}));throw new Error(t.message||e.statusText)}return e.json()}const p={async getMe(){const e=await fetch(`${f}/auth/me`,{headers:w()});return E(e)},async logout(){await fetch(`${f}/auth/logout`,{method:"POST",headers:w()}),A()},async listInstances(){const e=await fetch(`${f}/admin/instances`,{headers:w()});return E(e)},async getInstance(e){const t=await fetch(`${f}/api/orderprocessing/${e}`,{headers:w()});return E(t)},async createInstance(e={}){const t=await fetch(`${f}/api/orderprocessing`,{method:"POST",headers:w(),body:JSON.stringify(e)});return E(t)},async executeTransition(e,t,n={}){const o=n,i=await fetch(`${f}/api/${e}`,{method:"POST",headers:w(),body:JSON.stringify({aggregate_id:t,data:o})});return E(i)}};window.api=p;Object.defineProperty(window,"currentInstance",{get:function(){return a}});window.setAuthToken=function(e){m=e};window.saveAuth=q;window.clearAuth=A;function S(e){const t=document.getElementById("app"),n=t.querySelector(".alert-error");n&&n.remove();const o=document.createElement("div");o.className="alert alert-error",o.textContent=e,t.insertBefore(o,t.firstChild),setTimeout(()=>o.remove(),5e3)}function _(e){const t=document.getElementById("app"),n=t.querySelector(".alert-success");n&&n.remove();const o=document.createElement("div");o.className="alert alert-success",o.textContent=e,t.insertBefore(o,t.firstChild),setTimeout(()=>o.remove(),3e3)}function H(e){if(!e)return"unknown";for(const[t,n]of Object.entries(e))if(n>0)return t;return"unknown"}function z(e){return`<span class="badge ${`badge-${e.toLowerCase().replace(/_/g,"-")}`}">${e.replace(/_/g," ")}</span>`}async function M(){const e=document.getElementById("app");e.innerHTML=`
    <div class="page">
  ...
  `;try{b=(await p.listInstances()).instances||[],de()}catch{document.getElementById("instances-list").innerHTML=`
    ...
    `;return}e.innerHTML=b.map(t=>{const n=H(t.state||t.places);return`
      <div class="entity-card" onclick="navigate('/orderprocessing/${t.id}')">
        ...
        <div class="entity-actions">
          <button class="btn btn-sm btn-secondary" onclick="event.stopPropagation(); navigate('/orderprocessing/${t.id}')">
            View
          </button>
        </div>
      </div>
  ...
  `;try{const o=await p.getInstance(t);a={id:o.aggregate_id||t,version:o.version,state:o.state,displayState:o.state,places:o.places,enabled:o.enabled||o.enabled_transitions||[]},window.currentInstanceState=a.state,k()}catch(o){document.getElementById("instance-detail").innerHTML=`
    ...
    `}}function k(){const e=document.getElementById("instance-detail");if(!e||!a)return;const t=H(a.places),n=a.enabled||[],o=j;e.innerHTML=`
    <div class="card">
  ...
  `}function le(e){return!e||Object.keys(e).length===0?'<p style="color: #999;">No state data</p>':Object.entries(e).map(([t,n])=>{if(typeof n=="object"&&n!==null){const o=Object.entries(n);return o.length===0?`
        ...
        <div class="detail-field">
          ...
          <dd>
            <div class="nested-state">
              ${o.map(([i,s])=>{if(typeof s=="object"&&s!==null){const d=Object.entries(s);return d.length===0?`
                      ...
                      <div class="nested-state" style="margin-left: 1rem;">
                        ${d.map(([r,h])=>`
                          <div class="state-entry">
                            ...
                            <span class="state-value">${P(t,h)}</span>
                          </div>
                        `).join("")}
                      </div>
                    </div>
                  ...
                  <div class="state-entry">
                    <span class="state-key">${i}</span>
                    <span class="state-value">${P(t,s)}</span>
                  </div>
                `}).join("")}
            </div>
          </dd>
    ...
    `}).join("")}function O(e){return e.replace(/_/g," ").replace(/\b\w/g,t=>t.toUpperCase())}function P(e,t){return se(e),`<strong>${t}</strong>`}function ue(){if(typeof wallet<"u"&&wallet.getAccount){const e=wallet.getAccount();return(e==null?void 0:e.address)||null}return null}function pe(e,t){return e?e==="wallet"?ue()||"":e==="user"?(c==null?void 0:c.id)||(c==null?void 0:c.login)||"":(e.startsWith("balances.")||e.includes("."),""):""}function me(){return`
  ...
  `}let T=null;function he(e){const t=j.find(o=>o.id===e);if(!t)return;T=e,document.getElementById("action-modal-title").textContent=t.name;const n=t.fields.map(o=>{const s=pe(o.autoFill,a==null?void 0:a.state)||o.defaultValue||"",d=o.required?"required":"";let r="";if(o.type==="amount")r=`
        <input
          type="number"
          name="${o.name}"
          value="${s}"
      ...
      `;else if(o.type==="address"){const h=ge();h.length>0?r=`
          <select name="${o.name}" ${d} class="form-control">
            <option value="">Select address...</option>
            ${h.map(g=>`
              <option value="${g.address}" ${g.address===s?"selected":""}>
  ...
  `).join(""),document.body.appendChild(s),setTimeout(()=>{document.addEventListener("click",function d(r){s.contains(r.target)||(s.remove(),document.removeEventListener("click",d))})},0)};window.selectAddress=function(e,t){const n=document.querySelector(`[name="${e}"]`);n&&(n.value=t,n.dispatchEvent(new Event("input",{bubbles:!0})));const o=document.querySelector(".address-picker-dropdown");o&&o.remove()};async function fe(){const e=document.getElementById("app");e.innerHTML=`
    <div class="page">
      <div class="page-header">
        <div>
          <button class="btn btn-link" onclick="navigate('/orderprocessing')" style="margin-left: -0.5rem">
  ...
  `;try{const[t,n]=await Promise.all([fetch(`${f}/admin/stats`,{headers:w()}).then(i=>i.json()).catch(()=>null),p.listInstances()]);t?document.getElementById("admin-stats").innerHTML=`
