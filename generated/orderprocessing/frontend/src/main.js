...
function hasCustomDashboard() {
  return customElements.get('pf-order-processing-dashboard') !== undefined
}
...
const api = {
  ...
  async getInstance(id) {
    const response = await fetch(`${API_BASE}/api/orderprocessing/${id}`, { headers: getHeaders() })
    return handleResponse(response)
  },

  async createInstance(data = {}) {
    const response = await fetch(`${API_BASE}/api/orderprocessing`, {
      method: 'POST',
      headers: getHeaders(),
...
async function renderListPage() {
  ...
  if (hasCustomDashboard()) {
    app.innerHTML = `<pf-order-processing-dashboard></pf-order-processing-dashboard>`
    return
  }
...
window.pilot = {
  ...
  async getEvents() {
    if (!currentInstance) throw new Error('No current instance')
    const response = await fetch(`${API_BASE}/api/orderprocessing/${currentInstance.id}/events`, {
      headers: getHeaders()
    })
  ...
  getPlaces() {
    return [
      { id: 'received', name: 'Received', initial: 1 },
      { id: 'validated', name: 'Validated', initial: 0 },
      { id: 'rejected', name: 'Rejected', initial: 0 },
      { id: 'paid', name: 'Paid', initial: 0 },
      { id: 'shipped', name: 'Shipped', initial: 0 },
      { id: 'completed', name: 'Completed', initial: 0 },
    ]
  },
