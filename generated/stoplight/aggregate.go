...
func NewAggregate(id string) *Aggregate {
	...
	sm.RegisterHandler(EventTypeGo, func(state *State, event *eventsource.Event) error {
		return applyGo(state, event)
	})
	sm.RegisterHandler(EventTypeSlow, func(state *State, event *eventsource.Event) error {
		return applySlow(state, event)
	})
	sm.RegisterHandler(EventTypeStop, func(state *State, event *eventsource.Event) error {
		return applyStop(state, event)
	})
...
func applyGo(state *State, event *eventsource.Event) error {
...
func applySlow(state *State, event *eventsource.Event) error {
	// No data transformations for this transition
	_ = state
	_ = event
	return nil
...
func applyStop(state *State, event *eventsource.Event) error {
	// No data transformations for this transition
	_ = state
	_ = event
	return nil
