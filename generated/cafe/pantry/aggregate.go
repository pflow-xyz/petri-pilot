...
// State holds the aggregate state for pantry.
type State struct {
...
// Aggregate wraps a StateMachine with the pantry state.
...
func NewAggregate(id string) *Aggregate {
	...
	sm.AddTransition(eventsource.Transition{
		ID:        TransitionBrewEspresso,
		EventType: EventTypeBrewEspresso,
	...
	sm.AddTransition(eventsource.Transition{
		ID:        TransitionBrewLatte,
		EventType: EventTypeBrewLatte,
		Inputs: map[string]int{
			PlaceCoffeeBeans: 15,
			PlaceMilk:        50,
			PlaceCups:        1,
	...
	sm.AddTransition(eventsource.Transition{
		ID:        TransitionBrewCappuccino,
		EventType: EventTypeBrewCappuccino,
		Inputs: map[string]int{
			PlaceCoffeeBeans: 15,
			PlaceMilk:        30,
			PlaceCups:        1,
	...
	sm.RegisterHandler(EventTypeBrewEspresso, func(state *State, event *eventsource.Event) error {
		return applyBrewEspresso(state, event)
	})
	sm.RegisterHandler(EventTypeBrewLatte, func(state *State, event *eventsource.Event) error {
		return applyBrewLatte(state, event)
	})
	sm.RegisterHandler(EventTypeBrewCappuccino, func(state *State, event *eventsource.Event) error {
		return applyBrewCappuccino(state, event)
	})
...
func applyBrewEspresso(state *State, event *eventsource.Event) error {
...
func applyBrewLatte(state *State, event *eventsource.Event) error {
	// No data transformations for this transition
	_ = state
	_ = event
	return nil
...
func applyBrewCappuccino(state *State, event *eventsource.Event) error {
	// No data transformations for this transition
	_ = state
	_ = event
	return nil
...
func (app *Application) Load(ctx context.Context, id string) (*Aggregate, error) {
	events, err := app.store.Read(ctx, "pantry/"+id, 0)
	if err != nil {
...
func (app *Application) Execute(ctx context.Context, id, transitionID string, data any) (*Aggregate, error) {
	...
	_, err = app.store.Append(ctx, "pantry/"+id, agg.Version(), []*eventsource.Event{event})
	if err != nil {
...
func (app *Application) TruncateTo(ctx context.Context, id string, targetVersion int) (*Aggregate, error) {
	...
	if len(eventsToKeep) > 0 {
		if _, err := app.store.Append(ctx, "pantry/"+id, -1, eventsToKeep); err != nil {
