...
function hasCustomDashboard() {
  return customElements.get('pf-coffeeshop-dashboard') !== undefined
}
...
const TRANSITION_DEFS = [
  {
    id: 'order_espresso',
    name: 'Order Espresso',
    description: "Customer orders espresso",
    fields: [
    ]
  },
  {
    id: 'order_latte',
    name: 'Order Latte',
    description: "Customer orders latte",
    fields: [
    ]
  },
  {
    id: 'order_cappuccino',
    name: 'Order Cappuccino',
    description: "Customer orders cappuccino",
    fields: [
    ]
  },
  {
    id: 'make_espresso',
    name: 'Make Espresso',
    description: "Barista makes espresso",
    fields: [
    ]
  },
  {
    id: 'make_latte',
    name: 'Make Latte',
    description: "Barista makes latte",
    fields: [
    ]
  },
  {
    ...
    description: "Barista makes cappuccino",
    fields: [
    ]
  },
  {
    id: 'serve_espresso',
    name: 'Serve Espresso',
    description: "Serve espresso to customer",
    fields: [
    ]
  },
  {
    id: 'serve_latte',
    name: 'Serve Latte',
    description: "Serve latte to customer",
    fields: [
    ]
  },
  {
    id: 'serve_cappuccino',
    name: 'Serve Cappuccino',
    description: "Serve cappuccino to customer",
    fields: [
    ]
  },
  {
...
const api = {
  ...
  async getInstance(id) {
    const response = await fetch(`${API_BASE}/api/coffeeshop/${id}`, { headers: getHeaders() })
    return handleResponse(response)
  },
...
async function renderListPage() {
  ...
  if (hasCustomDashboard()) {
    app.innerHTML = `<pf-coffeeshop-dashboard></pf-coffeeshop-dashboard>`
    return
  }
...
function renderInstancesList() {
  ...
  container.innerHTML = instances.map(inst => {
    ...
    return `
      <div class="entity-card" onclick="navigate('/coffeeshop/${inst.id}')">
        ...
        <div class="entity-actions">
          <button class="btn btn-sm btn-secondary" onclick="event.stopPropagation(); navigate('/coffeeshop/${inst.id}')">
            View
          </button>
        </div>
      </div>
              ...
              ${instances.slice(0, 20).map(inst => {
                ...
                return `
                  <tr>
                    ...
                    <td><button class="btn btn-sm btn-link" onclick="navigate('/coffeeshop/${inst.id}')">View</button></td>
                  </tr>
                `
              }).join('')}
            </tbody>
...
window.handleSubmitCreate = async function(event) {
  ...
  try {
    ...
    navigate(`/coffeeshop/${result.aggregate_id || result.id}`)
  } catch (err) {
...
window.pilot = {
  ...
  async getEvents() {
    if (!currentInstance) throw new Error('No current instance')
    const response = await fetch(`${API_BASE}/api/coffeeshop/${currentInstance.id}/events`, {
      headers: getHeaders()
    })
  ...
  getPlaces() {
    return [
      { id: 'coffee_beans', name: 'CoffeeBeans', initial: 1000 },
      { id: 'milk', name: 'Milk', initial: 500 },
      { id: 'cups', name: 'Cups', initial: 200 },
      { id: 'orders_pending', name: 'OrdersPending', initial: 0 },
      { id: 'espresso_ready', name: 'EspressoReady', initial: 0 },
      { id: 'latte_ready', name: 'LatteReady', initial: 0 },
      { id: 'cappuccino_ready', name: 'CappuccinoReady', initial: 0 },
      { id: 'orders_complete', name: 'OrdersComplete', initial: 0 },
    ]
  },
