...
function hasCustomDashboard() {
  return customElements.get('pf-loan-application-dashboard') !== undefined
}
...
async function renderListPage() {
  ...
  if (hasCustomDashboard()) {
    app.innerHTML = `<pf-loan-application-dashboard></pf-loan-application-dashboard>`
    return
  }
...
function renderInstancesList() {
  ...
  container.innerHTML = instances.map(inst => {
    ...
    return `
      <div class="entity-card" onclick="navigate('/loan-application/${inst.id}')">
        ...
        <div class="entity-actions">
          <button class="btn btn-sm btn-secondary" onclick="event.stopPropagation(); navigate('/loan-application/${inst.id}')">
            View
          </button>
        </div>
      </div>
              ...
              ${instances.slice(0, 20).map(inst => {
                ...
                return `
                  <tr>
                    ...
                    <td><button class="btn btn-sm btn-link" onclick="navigate('/loan-application/${inst.id}')">View</button></td>
                  </tr>
                `
              }).join('')}
            </tbody>
...
window.handleSubmitCreate = async function(event) {
  ...
  try {
    ...
    navigate(`/loan-application/${result.aggregate_id || result.id}`)
  } catch (err) {
...
function handleRouteChange(event) {
  ...
  if (path === '/loan-application' || path === '/') {
    renderListPage()
  } else if (path === '/loan-application/new') {
    renderFormPage()
  } else if (path === '/loan-application/:id') {
    renderDetailPage()
  } else if (path === '/schema') {
    renderSchemaPage()
  } else {
