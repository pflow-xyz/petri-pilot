...
const TRANSITION_DEFS = [
  {
    ...
    description: "Initiate automated credit check",
    fields: [
    ]
  },
  {
    id: 'auto_approve',
    name: 'Auto Approve',
    description: "Automatic approval based on credit score",
    fields: [
    ]
  },
  {
  ...
  {
    id: 'underwriter_approve',
    name: 'Underwriter Approve',
    description: "Underwriter approves the application",
    fields: [
    ]
  },
  {
    id: 'underwriter_deny',
    name: 'Underwriter Deny',
    description: "Underwriter denies the application",
    fields: [
    ]
  },
  {
    id: 'auto_deny',
    name: 'Auto Deny',
    description: "Automatic denial based on credit score",
    fields: [
    ]
  },
  {
    ...
    description: "Finalize loan approval",
    fields: [
    ]
  },
  {
    id: 'disburse',
    name: 'Disburse',
    description: "Disburse loan funds to customer",
    fields: [
    ]
  },
  {
    id: 'start_repayment',
    name: 'Start Repayment',
    description: "Begin repayment period",
    fields: [
    ]
  },
  {
  ...
  },
  {
    id: 'complete',
    name: 'Complete',
    description: "Final payment received, loan complete",
    fields: [
    ]
  },
  {
...
const api = {
  ...
  async getInstance(id) {
    const response = await fetch(`${API_BASE}/api/loanapplication/${id}`, { headers: getHeaders() })
    return handleResponse(response)
  },

  async createInstance(data = {}) {
    const response = await fetch(`${API_BASE}/api/loanapplication`, {
      method: 'POST',
      headers: getHeaders(),
...
function renderInstancesList() {
  ...
  container.innerHTML = instances.map(inst => {
    ...
    return `
      <div class="entity-card" onclick="navigate('/loanapplication/${inst.id}')">
        ...
        <div class="entity-actions">
          <button class="btn btn-sm btn-secondary" onclick="event.stopPropagation(); navigate('/loanapplication/${inst.id}')">
            View
          </button>
        </div>
      </div>
              ...
              ${instances.slice(0, 20).map(inst => {
                ...
                return `
                  <tr>
                    ...
                    <td><button class="btn btn-sm btn-link" onclick="navigate('/loanapplication/${inst.id}')">View</button></td>
                  </tr>
                `
              }).join('')}
            </tbody>
...
window.handleSubmitCreate = async function(event) {
  ...
  try {
    ...
    navigate(`/loanapplication/${result.aggregate_id || result.id}`)
  } catch (err) {
...
function handleRouteChange(event) {
  ...
  if (path === '/loanapplication' || path === '/') {
    renderListPage()
  } else if (path === '/loanapplication/new') {
    renderFormPage()
  } else if (path === '/loanapplication/:id') {
    renderDetailPage()
...
window.pilot = {
  ...
  async getEvents() {
    if (!currentInstance) throw new Error('No current instance')
    const response = await fetch(`${API_BASE}/api/loanapplication/${currentInstance.id}/events`, {
      headers: getHeaders()
    })
  ...
  getPlaces() {
    return [
      { id: 'submitted', name: 'Submitted', initial: 1 },
      { id: 'credit_check', name: 'CreditCheck', initial: 0 },
      { id: 'auto_approved', name: 'AutoApproved', initial: 0 },
      { id: 'manual_review', name: 'ManualReview', initial: 0 },
      { id: 'approved', name: 'Approved', initial: 0 },
      { id: 'denied', name: 'Denied', initial: 0 },
      { id: 'disbursed', name: 'Disbursed', initial: 0 },
      { id: 'repaying', name: 'Repaying', initial: 0 },
      { id: 'paid_off', name: 'PaidOff', initial: 0 },
      { id: 'defaulted', name: 'Defaulted', initial: 0 },
    ]
  },
