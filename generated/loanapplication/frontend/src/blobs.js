...
// Blob storage helpers for loan-application
...
export async function getBlob(id) {
    const response = await fetch(`${API_BASE}/api/blobs/${id}`, {
        credentials: 'include'
    });
...
export async function getBlobMeta(id) {
    const response = await fetch(`${API_BASE}/api/blobs/${id}/meta`, {
        credentials: 'include'
    });
...
export async function listBlobs(owner = null) {
    const url = owner ? `${API_BASE}/api/blobs?owner=${encodeURIComponent(owner)}` : `${API_BASE}/api/blobs`;
