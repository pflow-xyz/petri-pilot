...
/**
 * Router configuration for loan-application application
...
export function navigate(path, state = {}) {
  ...
  if (!match) {
    ...
    const fallbackPath = '/loan-application'
  ...
  if (match.route.roles && match.route.roles.length > 0) {
    ...
    if (!user || !hasAnyRole(user, match.route.roles)) {
      ...
      navigate(`${getFrontendBase()}/loan-application`)
      return
    }
  }
...
export function initRouter() {
  ...
  if (match) {
    ...
    currentRoute = routes.find(r => r.path === '/loan-application') || routes[0]
    currentParams = {}
  }
}
