(function(){const t=document.createElement("link").relList;if(t&&t.supports&&t.supports("modulepreload"))return;for(const s of document.querySelectorAll('link[rel="modulepreload"]'))a(s);new MutationObserver(s=>{for(const o of s)if(o.type==="childList")for(const l of o.addedNodes)l.tagName==="LINK"&&l.rel==="modulepreload"&&a(l)}).observe(document,{childList:!0,subtree:!0});function n(s){const o={};return s.integrity&&(o.integrity=s.integrity),s.referrerPolicy&&(o.referrerPolicy=s.referrerPolicy),s.crossOrigin==="use-credentials"?o.credentials="include":s.crossOrigin==="anonymous"?o.credentials="omit":o.credentials="same-origin",o}function a(s){if(s.ep)return;s.ep=!0;const o=n(s);fetch(s.href,o)}})();const j=[{path:"/",component:"List",title:"loan-application"},{path:"/loanapplication",component:"List",title:"loan-application"},{path:"/loanapplication/new",component:"Form",title:"New loan-application"},{path:"/loanapplication/:id",component:"Detail",title:"loan-application Detail"},{path:"/admin",component:"AdminDashboard",title:"Admin Dashboard"},{path:"/admin/instances",component:"AdminInstances",title:"Instances"},{path:"/admin/instances/:id",component:"AdminInstance",title:"Instance Detail"}];let y=null,$={};function L(e){e=e||"/",e!=="/"&&e.endsWith("/")&&(e=e.slice(0,-1));for(const t of j){const n={};let a=t.path.replace(/[.*+?^${}()|[\]\\]/g,"\\$&");a=a.replace(/:[^/]+/g,"([^/]+)");const s=new RegExp(`^${a}$`),o=e.match(s);if(o)return(t.path.match(/:[^/]+/g)||[]).map(r=>r.slice(1)).forEach((r,h)=>{n[r]=decodeURIComponent(o[h+1])}),{route:t,params:n}}return null}function u(e,t={}){e.startsWith("/")||(e="/"+e);const n=L(e);if(!n){console.warn(`No route found for path: ${e}, falling back to list`),e="/loanapplication";const a=L(e);a&&(y=a.route,$=a.params,window.history.pushState(t,"",e),P());return}if(n.route.roles&&n.route.roles.length>0){const a=Q();if(!a||!X(a,n.route.roles)){console.warn("Access denied:",e),u("/loanapplication");return}}y=n.route,$=n.params,window.history.pushState(t,"",e),P()}window.addEventListener("popstate",()=>{const e=window.location.pathname,t=L(e);t?(y=t.route,$=t.params,P()):u("/loanapplication")});function Q(){const e=localStorage.getItem("auth");if(e)try{return JSON.parse(e).user}catch{return null}return null}function X(e,t){return!e||!e.roles?!1:t.some(n=>e.roles.includes(n))}function P(){window.dispatchEvent(new CustomEvent("route-change",{detail:{route:y,params:$}}))}function Z(){return $}function S(){return y}function ee(){const e=window.location.pathname,t=L(e);t?(y=t.route,$=t.params):(y=j.find(n=>n.path==="/loanapplication")||j[0],$={})}const J=[{id:"applicant",description:"Customer applying for a loan"},{id:"system",description:"Automated credit check and processing system"},{id:"underwriter",description:"Loan underwriter who reviews applications"},{id:"admin",description:"Full access to all operations"}],N={brand:"loan-application",items:[{label:"loan-application",path:"/loanapplication",icon:""},{label:"New",path:"/loanapplication/new",icon:"+"},{label:"Admin",path:"/admin",icon:""}]};let c=null,D=!1;async function V(){if(!D){D=!0;try{const e={},t=G();t&&(e.Authorization=`Bearer ${t}`);const n=await fetch("/api/navigation",{headers:e});n.ok?c=await n.json():c=N}catch{c=N}finally{D=!1}}}async function K(){c||await V();const e=window.location.pathname,t=ne(),n=(c==null?void 0:c.items)||N.items;return`
    <nav class="navigation">
      <div class="nav-brand">
        <a href="/loanapplication" onclick="handleNavClick(event, '/loanapplication')">
          ${(c==null?void 0:c.brand)||N.brand}
        </a>
      </div>
      <ul class="nav-menu">
        ${n.map(o=>`
  ...
  `}window.showLoginModal=function(){const e=document.getElementById("login-modal");e&&(e.style.display="flex")};window.hideLoginModal=function(){const e=document.getElementById("login-modal");e&&(e.style.display="none")};window.handleRoleLogin=async function(e){try{const t=await fetch("/api/debug/login",{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify({login:"pilot-user",roles:e})});if(!t.ok)throw new Error("Login failed");const n=await t.json();localStorage.setItem("auth",JSON.stringify(n)),hideLoginModal(),c=null,window.dispatchEvent(new CustomEvent("auth-change")),await F()}catch(t){console.error("Login error:",t),alert("Login failed. Please try again.")}};window.handleNavClick=function(e,t){e.preventDefault(),u(t)};window.handleLogout=async function(){try{const e=G();e&&await fetch("/auth/logout",{method:"POST",headers:{Authorization:`Bearer ${e}`}})}catch(e){console.error("Logout error:",e)}localStorage.removeItem("auth"),c=null,window.dispatchEvent(new CustomEvent("auth-change")),await F(),u("/loanapplication")};function ne(){const e=localStorage.getItem("auth");if(e)try{return JSON.parse(e).user}catch{return null}return null}function G(){const e=localStorage.getItem("auth");if(e)try{return JSON.parse(e).token}catch{return null}return null}async function F(){c=null,await V();const e=document.getElementById("nav");e&&(e.innerHTML=await K())}window.addEventListener("auth-change",async()=>{await F()});window.addEventListener("route-change",()=>{const e=window.location.pathname;document.querySelectorAll(".nav-menu li").forEach(t=>{t.classList.remove("active")}),document.querySelectorAll(".nav-menu a").forEach(t=>{const n=t.getAttribute("href");(n===e||n!=="/"&&e.startsWith(n))&&t.parentElement.classList.add("active")})});let W=[];async function ae(){try{const e=await fetch("/api/views");return e.ok?(W=await e.json(),W):(console.warn("Failed to load view definitions, using defaults"),[])}catch(e){return console.error("Error loading views:",e),[]}}const g="",ie="";function oe(e){return["amount","value","balance","total_supply","allowance"].some(n=>e.toLowerCase().includes(n))}let d=null,m=null,b=[],i=null;const I=[{id:"run_credit_check",name:"Run Credit Check",description:"Initiate automated credit check",fields:[]},{id:"auto_approve",name:"Auto Approve",description:"Automatic approval based on credit score",fields:[]},{id:"flag_for_review",name:"Flag For Review",description:"Flag application for manual review",fields:[]},{id:"underwriter_approve",name:"Underwriter Approve",description:"Underwriter approves the application",fields:[]},{id:"underwriter_deny",name:"Underwriter Deny",description:"Underwriter denies the application",fields:[]},{id:"auto_deny",name:"Auto Deny",description:"Automatic denial based on credit score",fields:[]},{id:"finalize_approval",name:"Finalize Approval",description:"Finalize loan approval",fields:[]},{id:"disburse",name:"Disburse",description:"Disburse loan funds to customer",fields:[]},{id:"start_repayment",name:"Start Repayment",description:"Begin repayment period",fields:[]},{id:"make_payment",name:"Make Payment",description:"Customer makes a payment",fields:[]},{id:"complete",name:"Complete",description:"Final payment received, loan complete",fields:[]},{id:"mark_default",name:"Mark Default",description:"Mark loan as defaulted",fields:[]}];function se(){const e=localStorage.getItem("auth");if(e)try{const t=JSON.parse(e);if(t.expires_at&&new Date(t.expires_at)>new Date)return m=t.token,d=t.user,!0;localStorage.removeItem("auth")}catch{localStorage.removeItem("auth")}return!1}function B(e){localStorage.setItem("auth",JSON.stringify(e)),m=e.token,d=e.user,window.dispatchEvent(new CustomEvent("auth-change"))}function A(){localStorage.removeItem("auth"),m=null,d=null,window.dispatchEvent(new CustomEvent("auth-change"))}function re(){const e=localStorage.getItem("auth");if(e)try{const t=JSON.parse(e);return m=t.token,d=t.user,!0}catch{return!1}return m=null,d=null,!1}window.addEventListener("auth-change",()=>{re()});function w(){const e={"Content-Type":"application/json"};return m&&(e.Authorization=`Bearer ${m}`),e}async function E(e){if(e.status===401)throw A(),k("Session expired. Please log in again."),new Error("Unauthorized");if(!e.ok){const t=await e.json().catch(()=>({}));throw new Error(t.message||e.statusText)}return e.json()}const p={async getMe(){const e=await fetch(`${g}/auth/me`,{headers:w()});return E(e)},async logout(){await fetch(`${g}/auth/logout`,{method:"POST",headers:w()}),A()},async listInstances(){const e=await fetch(`${g}/admin/instances`,{headers:w()});return E(e)},async getInstance(e){const t=await fetch(`${g}/api/loanapplication/${e}`,{headers:w()});return E(t)},async createInstance(e={}){const t=await fetch(`${g}/api/loanapplication`,{method:"POST",headers:w(),body:JSON.stringify(e)});return E(t)},async executeTransition(e,t,n={}){const a=n,s=await fetch(`${g}/api/${e}`,{method:"POST",headers:w(),body:JSON.stringify({aggregate_id:t,data:a})});return E(s)}};window.api=p;Object.defineProperty(window,"currentInstance",{get:function(){return i}});window.setAuthToken=function(e){m=e};window.saveAuth=B;window.clearAuth=A;function k(e){const t=document.getElementById("app"),n=t.querySelector(".alert-error");n&&n.remove();const a=document.createElement("div");a.className="alert alert-error",a.textContent=e,t.insertBefore(a,t.firstChild),setTimeout(()=>a.remove(),5e3)}function q(e){const t=document.getElementById("app"),n=t.querySelector(".alert-success");n&&n.remove();const a=document.createElement("div");a.className="alert alert-success",a.textContent=e,t.insertBefore(a,t.firstChild),setTimeout(()=>a.remove(),3e3)}function z(e){if(!e)return"unknown";for(const[t,n]of Object.entries(e))if(n>0)return t;return"unknown"}function H(e){return`<span class="badge ${`badge-${e.toLowerCase().replace(/_/g,"-")}`}">${e.replace(/_/g," ")}</span>`}async function R(){const e=document.getElementById("app");e.innerHTML=`
    <div class="page">
  ...
  `;try{b=(await p.listInstances()).instances||[],le()}catch{document.getElementById("instances-list").innerHTML=`
    ...
    `}}function le(){const e=document.getElementById("instances-list");if(e){if(b.length===0){e.innerHTML=`
      <div class="empty-state">
        <h3>No instances yet</h3>
        <p>Create your first instance to get started.</p>
        <button class="btn btn-primary" onclick="handleCreateNew()" style="margin-top: 1rem">+ Create New</button>
      </div>
    `;return}e.innerHTML=b.map(t=>{const n=z(t.state||t.places);return`
      <div class="entity-card" onclick="navigate('/loanapplication/${t.id}')">
        <div class="entity-info">
          <h3>${t.id}</h3>
          <div class="entity-meta">
            ${H(n)} &middot; Version ${t.version||0}
          </div>
        </div>
        <div class="entity-actions">
          <button class="btn btn-sm btn-secondary" onclick="event.stopPropagation(); navigate('/loanapplication/${t.id}')">
            View
          </button>
        </div>
      </div>
    ...
    <div class="page">
      <div class="page-header">
        <div>
          <button class="btn btn-link" onclick="navigate('/loanapplication')" style="margin-left: -0.5rem">
            &larr; Back to List
          </button>
  ...
  `;try{const a=await p.getInstance(t);i={id:a.aggregate_id||t,version:a.version,state:a.state,displayState:a.state,places:a.places,enabled:a.enabled||a.enabled_transitions||[]},window.currentInstanceState=i.state,x()}catch(a){document.getElementById("instance-detail").innerHTML=`
    ...
    `}}function x(){const e=document.getElementById("instance-detail");if(!e||!i)return;const t=z(i.places),n=i.enabled||[],a=I;e.innerHTML=`
    <div class="card">
    ...
    <div class="card">
      ...
      <div class="detail-list">
        ${ce(i.displayState||i.state)}
      </div>
    </div>
  `}function ce(e){return!e||Object.keys(e).length===0?'<p style="color: #999;">No state data</p>':Object.entries(e).map(([t,n])=>{if(typeof n=="object"&&n!==null){const a=Object.entries(n);return a.length===0?`
          <div class="detail-field">
            <dt>${M(t)}</dt>
        ...
        <div class="detail-field">
          ...
          <dd>
            <div class="nested-state">
              ${a.map(([s,o])=>{if(typeof o=="object"&&o!==null){const l=Object.entries(o);return l.length===0?`
                      ...
                      <div class="nested-state" style="margin-left: 1rem;">
                        ${l.map(([r,h])=>`
                          <div class="state-entry">
                            ...
                            <span class="state-value">${O(t,h)}</span>
                          </div>
                        `).join("")}
                      </div>
                    </div>
                  ...
                  <div class="state-entry">
                    <span class="state-key">${s}</span>
                    <span class="state-value">${O(t,o)}</span>
                  </div>
                `}).join("")}
            </div>
          </dd>
    ...
    `}).join("")}function M(e){return e.replace(/_/g," ").replace(/\b\w/g,t=>t.toUpperCase())}function O(e,t){return oe(e),`<strong>${t}</strong>`}function ue(){if(typeof wallet<"u"&&wallet.getAccount){const e=wallet.getAccount();return(e==null?void 0:e.address)||null}return null}function pe(e,t){return e?e==="wallet"?ue()||"":e==="user"?(d==null?void 0:d.id)||(d==null?void 0:d.login)||"":(e.startsWith("balances.")||e.includes("."),""):""}function me(){return`
  ...
  `}let T=null;function he(e){const t=I.find(a=>a.id===e);if(!t)return;T=e,document.getElementById("action-modal-title").textContent=t.name;const n=t.fields.map(a=>{const o=pe(a.autoFill,i==null?void 0:i.state)||a.defaultValue||"",l=a.required?"required":"";let r="";if(a.type==="amount")r=`
        <input
          type="number"
          name="${a.name}"
          value="${o}"
  ...
  `).join(""),document.body.appendChild(o),setTimeout(()=>{document.addEventListener("click",function l(r){o.contains(r.target)||(o.remove(),document.removeEventListener("click",l))})},0)};window.selectAddress=function(e,t){const n=document.querySelector(`[name="${e}"]`);n&&(n.value=t,n.dispatchEvent(new Event("input",{bubbles:!0})));const a=document.querySelector(".address-picker-dropdown");a&&a.remove()};async function ge(){const e=document.getElementById("app");e.innerHTML=`
    <div class="page">
      <div class="page-header">
        <div>
          <button class="btn btn-link" onclick="navigate('/loanapplication')" style="margin-left: -0.5rem">
      ...
      <div class="card">
        <form id="create-form" onsubmit="handleSubmitCreate(event)">
          ...
          <div class="form-actions">
            ...
            <button type="button" class="btn btn-secondary" onclick="navigate('/loanapplication')">Cancel</button>
          </div>
        </form>
      </div>
    </div>
