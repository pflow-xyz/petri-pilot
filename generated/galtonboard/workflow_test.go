...
func TestWorkflowConstants(t *testing.T) {
	...
	places := AllPlaces()
	if len(places) != 45 {
		t.Errorf("expected 45 places, got %d", len(places))
	}

	// Verify transition constants
	transitions := AllTransitions()
	if len(transitions) != 72 {
		t.Errorf("expected 72 transitions, got %d", len(transitions))
	}
}
...
func TestInitialPlaces(t *testing.T) {
	...
	if initial[PlaceTop] != 256 {
		t.Errorf("expected PlaceTop initial = 256, got %d", initial[PlaceTop])
	}
}
...
func TestInitialState(t *testing.T) {
	...
	if places[PlaceTop] != 256 {
		t.Errorf("expected PlaceTop = 256, got %d", places[PlaceTop])
	}
}
