...
/**
 * Router configuration for erc20-token application
...
export function navigate(path, state = {}) {
  ...
  if (!match) {
    ...
    const fallbackPath = '/erc20-token'
  ...
  if (match.route.roles && match.route.roles.length > 0) {
    ...
    if (!user || !hasAnyRole(user, match.route.roles)) {
      ...
      navigate(`${getFrontendBase()}/erc20-token`)
      return
    }
  }
...
export function initRouter() {
  ...
  if (match) {
    ...
    currentRoute = routes.find(r => r.path === '/erc20-token') || routes[0]
    currentParams = {}
  }
}
