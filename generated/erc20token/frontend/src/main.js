...
const api = {
  ...
  async getInstance(id) {
    const response = await fetch(`${API_BASE}/api/erc20token/${id}`, { headers: getHeaders() })
    return handleResponse(response)
  },

  async createInstance(data = {}) {
    const response = await fetch(`${API_BASE}/api/erc20token`, {
      method: 'POST',
      headers: getHeaders(),
...
window.pilot = {
  ...
  async getEvents() {
    if (!currentInstance) throw new Error('No current instance')
    const response = await fetch(`${API_BASE}/api/erc20token/${currentInstance.id}/events`, {
      headers: getHeaders()
    })
  ...
  getPlaces() {
    return [
      { id: 'total_supply', name: 'TotalSupply', initial: 0 },
      { id: 'balances', name: 'Balances', initial: 0 },
      { id: 'allowances', name: 'Allowances', initial: 0 },
    ]
  },
