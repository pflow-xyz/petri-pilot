...
function hasCustomDashboard() {
  return customElements.get('pf-test-access-dashboard') !== undefined
}
...
const api = {
  ...
  async getInstance(id) {
    const response = await fetch(`${API_BASE}/api/testaccess/${id}`, { headers: getHeaders() })
    return handleResponse(response)
  },

  async createInstance(data = {}) {
    const response = await fetch(`${API_BASE}/api/testaccess`, {
      method: 'POST',
      headers: getHeaders(),
...
async function renderListPage() {
  ...
  if (hasCustomDashboard()) {
    app.innerHTML = `<pf-test-access-dashboard></pf-test-access-dashboard>`
    return
  }
...
window.pilot = {
  ...
  async getEvents() {
    if (!currentInstance) throw new Error('No current instance')
    const response = await fetch(`${API_BASE}/api/testaccess/${currentInstance.id}/events`, {
      headers: getHeaders()
    })
