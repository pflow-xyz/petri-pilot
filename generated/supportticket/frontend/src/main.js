...
function hasCustomDashboard() {
  return customElements.get('pf-support-ticket-dashboard') !== undefined
}
...
const TRANSITION_DEFS = [
  ...
  },
  {
    id: 'customer_reply',
    name: 'Customer Reply',
    description: "Customer provides requested information",
    fields: [
    ]
  },
  {
    id: 'resolve',
    name: 'Resolve',
    description: "Mark issue as resolved from in_progress",
    fields: [
    ]
  },
  {
    id: 'resolve_escalated',
    name: 'Resolve Escalated',
    description: "Mark escalated issue as resolved",
    fields: [
    ]
  },
  {
  ...
  },
  {
    id: 'reopen',
    name: 'Reopen',
    description: "Customer reopens a closed ticket",
    fields: [
    ]
  },
]
...
async function renderListPage() {
  ...
  if (hasCustomDashboard()) {
    app.innerHTML = `<pf-support-ticket-dashboard></pf-support-ticket-dashboard>`
    return
  }
...
function renderInstancesList() {
  ...
  container.innerHTML = instances.map(inst => {
    ...
    return `
      <div class="entity-card" onclick="navigate('/support-ticket/${inst.id}')">
        ...
        <div class="entity-actions">
          <button class="btn btn-sm btn-secondary" onclick="event.stopPropagation(); navigate('/support-ticket/${inst.id}')">
            View
          </button>
        </div>
      </div>
              ...
              ${instances.slice(0, 20).map(inst => {
                ...
                return `
                  <tr>
                    ...
                    <td><button class="btn btn-sm btn-link" onclick="navigate('/support-ticket/${inst.id}')">View</button></td>
                  </tr>
                `
              }).join('')}
            </tbody>
...
window.handleSubmitCreate = async function(event) {
  ...
  try {
    ...
    navigate(`/support-ticket/${result.aggregate_id || result.id}`)
  } catch (err) {
...
function handleRouteChange(event) {
  ...
  if (path === '/support-ticket' || path === '/') {
    renderListPage()
  } else if (path === '/support-ticket/new') {
    renderFormPage()
  } else if (path === '/support-ticket/:id') {
    renderDetailPage()
  } else if (path === '/schema') {
    renderSchemaPage()
  } else {
