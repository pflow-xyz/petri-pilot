...
/**
 * Router configuration for support-ticket application
...
export function navigate(path, state = {}) {
  ...
  if (!match) {
    ...
    const fallbackPath = '/support-ticket'
  ...
  if (match.route.roles && match.route.roles.length > 0) {
    ...
    if (!user || !hasAnyRole(user, match.route.roles)) {
      ...
      navigate(`${getFrontendBase()}/support-ticket`)
      return
    }
  }
...
export function initRouter() {
  ...
  if (match) {
    ...
    currentRoute = routes.find(r => r.path === '/support-ticket') || routes[0]
    currentParams = {}
  }
}
