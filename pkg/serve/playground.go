...
<head>
  ...
  <style>
    ...
    #models-panel {
      ...
      font-family: 'Source Code Pro', monospace; font-size: 13px;
      transition: width 0.15s ease; border-left: none;
      display: flex; flex-direction: row;
    }
    #models-panel.open { width: 520px; overflow: visible; border-left: 1px solid rgba(255,255,255,0.06); }
    #models-panel.dragging { transition: none; }
    #models-panel-resize {
      position: absolute; left: -4px; top: 0; bottom: 0; width: 8px;
      cursor: ew-resize; z-index: 20; background: transparent;
    }
    #models-panel-resize:hover, #models-panel-resize.active {
...
<body>
  ...
  <div id="models-panel"><div id="models-panel-resize"></div><div id="models-panel-inner"><div class="mp-header"><span>Models</span></div><div id="models-content"></div></div></div>
  <script>
  ...
  (function() {
    ...
    var modelsPanelInner = document.getElementById('models-panel-inner');
    var modelsContent = document.getElementById('models-content');
    var modelsHeader = modelsPanelInner.querySelector('.mp-header');
    var modelsCache = {};
    ...
    function injectModelsTab() {
      ...
      // Find the Docs tab too
      var docsTab = null;
      for (var i = 0; i < allDivs.length; i++) {
        if (allDivs[i].textContent.trim() === 'Docs') {
          docsTab = allDivs[i];
          break;
        }
      }

      // Find the Schema/Docs panel container (it contains the actual panel content)
      var docExplorerWrap = null;
      function findDocExplorer() {
        docExplorerWrap = document.querySelector('.docExplorerWrap');
        if (!docExplorerWrap) setTimeout(findDocExplorer, 500);
      }
      findDocExplorer();

      // Close Models panel and show Schema/Docs when their tabs are clicked
      function closeModelsPanel() {
        modelsPanel.classList.remove('open');
        btn.classList.remove('active');
        // Show the Schema/Docs panel
        if (docExplorerWrap) docExplorerWrap.style.display = '';
      }

      schemaTab.addEventListener('click', closeModelsPanel);
      if (docsTab) docsTab.addEventListener('click', closeModelsPanel);

      // When Models is clicked, hide Schema/Docs panel and show Models
      btn.addEventListener('click', function() {
        var isOpen = modelsPanel.classList.toggle('open');
        btn.classList.toggle('active', isOpen);

        // Hide/show the Schema/Docs panel
        if (docExplorerWrap) {
          docExplorerWrap.style.display = isOpen ? 'none' : '';
        }

        if (isOpen) loadModelsList();
      });

      // Position the panel in the editor area
      var editorArea = document.querySelector('.sc-VJcYb');
      if (editorArea) {
      ...
      // Set up drag-to-resize
      var resizeHandle = document.getElementById('models-panel-resize');
      ...
      resizeHandle.addEventListener('mousedown', function(e) {
        isDragging = true;
        startX = e.clientX;
        startWidth = modelsPanel.offsetWidth;
        modelsPanel.classList.add('dragging');
      ...
      document.addEventListener('mousemove', function(e) {
        ...
        var newWidth = Math.max(280, Math.min(800, startWidth + delta));
        modelsPanel.style.width = newWidth + 'px';
      });

      document.addEventListener('mouseup', function() {
        if (isDragging) {
          isDragging = false;
          modelsPanel.classList.remove('dragging');
...
<body>
  ...
  (function() {
    ...
    fetch('/' + encodeURIComponent(modelName) + '/api/schema' + extraParams, {cache: 'no-store'})
      .then(function(r) {
    ...
    function convertToPflowFormat(model) {
      ...
      for (var i = 0; i < places.length; i++) {
        ...
        pflowPlaces[p.id] = {
          ...
          capacity: [null],  // Unlimited capacity (null) for proper token flow
          offset: 0
        };
      }
      ...
      for (var i = 0; i < transitions.length; i++) {
        ...
        pflowTransitions[t.id] = {
          ...
          x: t.x || 0,
          y: t.y || 0,
          role: 'default',  // Required for transition enablement display
          offset: 0
        };
      }
