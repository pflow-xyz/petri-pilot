...
// TestHighCardKickerScoring verifies the kicker scoring Petri net mechanism
func TestHighCardKickerScoring(t *testing.T) {
	model := buildPokerHandModel("Ah,Kd", "Qs,Jc,9h")
	...
	for _, tr := range transitions {
		id := tr["id"].(string)
		if strings.HasPrefix(id, "hc_") {
			hcTransitions++
		}
	}
	if hcTransitions != 52 {
		t.Errorf("Expected 52 hc_* transitions, got %d", hcTransitions)
	}

	// Verify arc structure for a specific transition (hc_A♥)
	hcAhInputs := []string{}
	hcAhOutputWeight := 0
	for _, arc := range arcs {
		from := arc["from"].(string)
		to := arc["to"].(string)
		if to == "hc_A♥" {
			hcAhInputs = append(hcAhInputs, from)
		}
		if from == "hc_A♥" && to == "kicker_score" {
			if w, ok := arc["weight"]; ok {
				hcAhOutputWeight = w.(int)
			}
		}
	}
	...
	for _, tc := range testCases {
		t.Run(tc.name, func(t *testing.T) {
			m := buildPokerHandModel(tc.hole, tc.board)
			trs := m["transitions"].([]map[string]any)
			as := m["arcs"].([]map[string]any)
			ps := m["places"].([]map[string]any)

			// Find which cards are in hand (initial=1 on card places)
			cardsInHand := map[string]bool{}
			for _, p := range ps {
				id := p["id"].(string)
				if isCardPlace(id) && p["initial"].(int) == 1 {
					cardsInHand[id] = true
				}
			}

			// Calculate expected kicker_score by summing weights of enabled hc_ transitions
			score := 0
			for _, tr := range trs {
				id := tr["id"].(string)
				if !strings.HasPrefix(id, "hc_") {
				...
				// Check if this transition's card is in hand
				for _, arc := range as {
					from := arc["from"].(string)
					to := arc["to"].(string)
					if to == id && cardsInHand[from] {
						// This card is in hand, find its output weight
						for _, outArc := range as {
							if outArc["from"].(string) == id && outArc["to"].(string) == "kicker_score" {
								if w, ok := outArc["weight"]; ok {
									score += w.(int)
								}
							}
						}
			...
			if score != tc.expected {
				t.Errorf("Expected kicker score %d, got %d", tc.expected, score)
			}
			t.Logf("%s: kicker_score=%d", tc.name, score)
		})
	}
