...
func buildDeckTrackerModel(holeStr, communityStr string) map[string]interface{} {
	...
	places = append(places,
		map[string]interface{}{"id": "deck_remaining", "initial": deckCount, "x": 120, "y": 60},
		map[string]interface{}{"id": "hole_cards", "initial": holeCount, "x": 300, "y": 60},
		map[string]interface{}{"id": "community_cards", "initial": communityCount, "x": 480, "y": 60},
	)

	// Card places organized in a grid by suit and rank
	// Layout: 13 cards per suit, 4 suits, with suit labels on the left
	cardStartY := 160
	cardSpacingX := 50
	cardSpacingY := 90
	...
	for suitIdx, suit := range suits {
		for rankIdx, rank := range ranks {
			cardID := rank + suit
			x := 100 + rankIdx*cardSpacingX
			y := cardStartY + suitIdx*cardSpacingY
	...
	// Add suit labels on the right side of each row
	for suitIdx, suit := range suits {
		places = append(places, map[string]interface{}{
			"id":      suitNames[suit],
			"initial": 0,
			"x":       100 + 13*cardSpacingX + 40,
			"y":       cardStartY + suitIdx*cardSpacingY,
		})
	}
	...
	transitions := []map[string]interface{}{
		{"id": "deal_hole", "x": 210, "y": 60},
		{"id": "deal_community", "x": 390, "y": 60},
	}
