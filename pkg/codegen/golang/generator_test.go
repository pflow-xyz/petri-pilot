...
func TestGenerateFiles(t *testing.T) {
	...
	// debug.go (when debug is enabled), sla.go (when SLA is configured)
	expectedFiles := []string{"go.mod", "main.go", "workflow.go", "events.go", "aggregate.go", "api.go", "openapi.yaml", "config.go", "workflow_test.go", "auth.go", "middleware.go", "permissions.go", "views.go", "api_events.go", "navigation.go", "admin.go", "debug.go", "sla.go"}
